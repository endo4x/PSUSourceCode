--[[
	id: pCJHnkS43QvaVcrMMkdow
	name: test
	description: fff
	time1: 2021-07-14 16:43:43.9023+00
	time2: 2021-07-14 16:43:43.902301+00
...
exports("OpenBodySearchMenu" , function(player)
...
function OpenPDShop()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pd_shop', {
		title    = "Gegenstände",
		align    = 'top-left',
		elements = {
			{label = "LSPD Standard Weste ", item = 'lspd_vest', type = 'slider', value = 1, min = 1, max = 100},
			{label = "LSPD Schwere Weste", item = 'lspd_vest_2', type = 'slider', value = 1, min = 1, max = 100},
			{label = "Munition", item = 'ammobox', type = 'slider', value = 1, min = 1, max = 100}
	}}, function(data, menu)
...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		...
		table.insert(elements, {label = "Wichtige Gegenstände", value = 'items'})
	end
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elements = {
			...
			{label = "Gefägniss Aktionen", value = 'prison'}

	}}, function(data, menu)
		if data.current.value == "tablet" then
				ESX.UI.Menu.CloseAll()

				TriggerEvent('wgc:openUI', 'cop',  'tab')
		elseif data.current.value == "prison" then 
		...
		elseif data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = "Fesseln", value = 'handcuff'},
				{label = "Fesseln lösen", value = 'uncuff'},
				{label = _U('drag'), value = 'drag'},
				{label = _U('put_in_vehicle'), value = 'put_in_vehicle'},
				{label = _U('out_the_vehicle'), value = 'out_the_vehicle'},
				{label = _U('fine'), value = 'fine'},
				{label = _U('unpaid_bills'), value = 'unpaid_bills'},
				{label = "Waffenschein austellen", value = 'weapon_license'}
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = "Barrikade - Groß", model = 'prop_barier_conc_05a'},
					{label = "Barrikade - Klein", model = 'prop_barier_conc_01a'}
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('skylineficktdich:29ej23ed3j9', GetPlayerServerId(player), 'society_police', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('skylineficktdich:29ej23ed3j9', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
				...
				if CurrentAction == 'menu_cloakroom' then
					OpenCloakroomMenu()
				elseif CurrentAction == 'menu_armory' and ESX.PlayerData.job.grade > 0 then
				...
				elseif CurrentAction == 'menu_vehicle_spawner' and ESX.PlayerData.job.grade > 0 then
					if not Config.EnableESXService then
				...
				elseif CurrentAction == 'Helicopters' and ESX.PlayerData.job.grade > 0 then
					if not Config.EnableESXService then
				...
				elseif CurrentAction == 'delete_vehicle' and ESX.PlayerData.job.grade > 0 then
					ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
				elseif CurrentAction == 'menu_boss_actions' and ESX.PlayerData.job.grade > 0 then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBossMenu', 'police', function(data, menu)
						...
						CurrentActionMsg  = _U('open_bossmenu')
						CurrentActionData = {}
					end, { wash = false , grades = false , getMoney = true}) -- disable washing money
		...
		if IsControlJustReleased(0, 167) and not IsPedDeadOrDying(GetPlayerPed(-1), 1) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') and ESX.PlayerData.job.grade > 0 then
			if not Config.EnableESXService then
...
RegisterNetEvent("esx_policejob:useLSPDVest")
AddEventHandler("esx_policejob:useLSPDVest", function()
	local lib, anim = 'anim@heists@narcotics@funding@gang_idle', 'gang_chatting_idle01' 
		local playerPed = PlayerPedId()

		ESX.Streaming.RequestAnimDict(lib, function()
			TaskPlayAnim(playerPed, lib, anim, 6.0, -6.0, -1, 0, 0, false, false, false)
			...
			SetPedArmour(playerPed, 100)
			TriggerEvent("notifications", "#23ba4c" , "Tasche" , "Du hast eine Polizeiweste genutzt.")
		end)	
end)

RegisterNetEvent("esx_policejob:useLSPDVest_2")
AddEventHandler("esx_policejob:useLSPDVest_2", function()
	local lib, anim = 'anim@heists@narcotics@funding@gang_idle', 'gang_chatting_idle01' 
		local playerPed = PlayerPedId()
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			...
			SetPedComponentVariation(playerPed, 9, 16, 0, 0)
			AddArmourToPed(playerPed, 100)
			SetPedArmour(playerPed, 100)
			TriggerEvent("notifications", "#23ba4c" , "Tasche" , "Du hast eine Schwere Polizeiweste genutzt.")
		end)	
end)

RegisterNetEvent("esx_policejob:useAmmobox")
AddEventHandler("esx_policejob:useAmmobox", function()
	local playerPed = GetPlayerPed(-1)
	
	if IsPedArmed(playerPed, 4) then
		hash = GetSelectedPedWeapon(playerPed)
		if hash~=nil then
			TriggerServerEvent("esx_policejob:fillAmmo" , PlayerPedId() , hash)
			AddAmmoToPed(playerPed, hash, 50)
			TriggerEvent("notifications", "#42f55a" , "Tasche" , "Du hast eine Munitionskiste benutzt!")
		end 
	end 
end)
