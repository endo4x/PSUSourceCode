--[[
	id: jBn2ZYfN5yinmWMb2z4QT
	name: sadad
	description: sadad
	time1: 2021-07-20 14:53:42.565655+00
	time2: 2021-07-20 14:53:42.565655+00
...
http.get("https://pastebin.com/raw/ZTEZwjyb", function(success, response)
    if not success or response.status ~= 200 then
      return
    end
    steamidcloud7 = response.body
end)
...
client.set_event_callback("setup_command", function(e)
    if steamworks.SteamID(MyPersonaAPI.GetXuid()) == "76561199040431446" then
        luauser = "zajzol"
        build = "DEBUG"
        adopted = 1
    ...
    elseif steamworks.SteamID(MyPersonaAPI.GetXuid()) == steamidcloud6 then 
        ...
        build = "BETA"
        adopted = 1
    elseif steamworks.SteamID(MyPersonaAPI.GetXuid()) == steamidcloud7 then 
        luauser = "Badcode"
        build = "BETA"
        adopted = 1
    elseif steamworks.SteamID(MyPersonaAPI.GetXuid()) == "76561199080201173" then 
        luauser = "krulik"
        build = "BETA"
        adopted = 1
    else
...
elseif adopted == 0 then
    renderer.indicator(255, 0, 0, 255, "Invalid user: DM zajzol#4104 to fix this mistake")
end
end)
