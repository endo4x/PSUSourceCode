--[[
	id: pPGmZ5dLzlkTks4rmMqym
	name: es_extended
	description: main.lua
	time1: 2021-07-12 20:35:08.56171+00
	time2: 2021-07-12 20:35:08.561711+00
...
function onPlayerJoined(playerId)
	...
	if identifier then
		if ESX.GetPlayerFromIdentifier(identifier) then
			...
			DropPlayer(playerId, 'Bu steam kimliği zaten sunucuda görünüyor!')
		else
		...
		DropPlayer(playerId, 'Steam kimliği bulunamadı! Steamin acik oldugundan emin olunuz!')
	end
end
...
AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
	...
	if identifier then
		if ESX.GetPlayerFromIdentifier(identifier) then
			deferrals.done('Bu steam kimliği zaten sunucuda görünüyor!')
		else
			deferrals.done()
		end
	else
		deferrals.done('Steam kimliği bulunamadı! Steamin acik oldugundan emin olunuz!')
	end
end)
...
function loadESXPlayer(identifier, playerId, first)
	...
	table.insert(tasks, function(cb) -- loadout = @loadout, 
		MySQL.Async.fetchAll('SELECT firstname, lastname, accounts, job, job_grade, `group`, position, inventory FROM users WHERE identifier = @identifier', {
			['@identifier'] = identifier
		}, function(result)
			...
			userData.charname = result[1].firstname .. ' ' .. result[1].lastname
			...
			if result[1].inventory and result[1].inventory ~= nil then
	...
	Async.parallel(tasks, function(results)
		...
		TriggerClientEvent('inventory:loadItemInfos', playerId, ESX.Items)
...
-- RegisterCommand('usableitems', function()
-- 	print(ESX.DumpTable(ESX.UsableItemsCallbacks))
-- end)
