--[[
	id: MdNXkI46YcPKXq4DMZtNU
	name: Script Hub
	description: Script Hub
	time1: 2021-05-08 17:00:58.856257+00
	time2: 2021-05-08 17:00:58.856257+00
	uploader: 817
...
do -- Example UI
	...
	local Name = "EvilHub.JSON"

	local Settings

	if not pcall(function() readfile(Name) end) then writefile(Name, game:service'HttpService':JSONEncode(SettingsT)) end
	...
	local Window = library:AddWindow("Evil Hub - Arsenal", {
		main_color = Color3.fromRGB(3, 3, 3),
		min_size = Vector2.new(500, 300),
		toggle_key = Enum.KeyCode.RightShift,
		can_resize = true,
	})
	local Tab = Window:AddTab("Visuals")
	local Tab2 = Window:AddTab("Gun Mods")
	local Tab3 = Window:AddTab("Character")
	local Tab4 = Window:AddTab("Player Cheats")
	...
	do -- Elements
		...
		local Switch = Tab:AddSwitch("ESP", function(bool)
			if bool then
				Settings.ESP = true
				Save()
				loadstring(game:HttpGet("https://pastebin.com/raw/fs8kpt3x"))()
                
			else
				Settings.ESP = false
				Save()
				loadstring(game:HttpGet("https://pastebin.com/raw/RympJ3GA"))()
				
			end
		end)
		Switch:Set(Settings.ESP)

		local Tracers =  Tab:AddSwitch("Tracers", function(bool)
			if bool then
				Settings.Tracers = true
				Save()
				loadstring(game:HttpGet("https://pastebin.com/raw/wGATFDwG"))()
				
			else
				Settings.Tracers = false 
				Save()
				loadstring(game:HttpGet("https://pastebin.com/raw/GHYcQcyj"))()
				
			end
		end)
		Tracers:Set(Settings.Tracers)
		local Chams = Tab:AddSwitch("Chams", function(bool)
			if bool then
				Settings.Chams = true
				Save()
				local Players = game:GetService'Players'
				local Player = Players.LocalPlayer
				local ShowInvisibleParts = true;

				function TrackHum(v, vHRP)
					...
					for ii,vv in pairs(vHRP.Parent:GetChildren()) do
						if vv:IsA'BasePart' then
							...
							Box.Adornee = vv
							Box.Size = vv:GetMass() > 1 and vv.Size-Vector3.new(.1,.1,.1) or Vector3.new(.4,.4,.4)
							Box.Transparency = (vv.Transparency < 1 or ShowInvisibleParts) and .5 or 1
							Box.AlwaysOnTop = true
							Box.ZIndex = 1

							Box.Color3 = (v.TeamColor == Player.TeamColor and Color3.new(0,1,0)) or Color3.new(1,0,0)

							InvisibruParts[#InvisibruParts+1] = Box

							local OnChange = not ShowInvisibleParts and vv.Changed:connect(function(Prop)
								if Prop == 'Transparency' then
									Box.Transparency = vv.Transparency < 1 and .5 or 1
									Box.Transparency = (v.TeamColor == Player.TeamColor and Color3.new(0,1,0)) or Color3.new(1,0,0)
								end
							end)
							local VChange = v.Changed:connect(function(Prop)
								if Prop == 'TeamColor' then
									Box.Color3 = (v.TeamColor == Player.TeamColor and Color3.new(0,1,0)) or Color3.new(1,0,0)
								end
							end)

							local PlayerChange = Player.Changed:connect(function(Prop)
								if Prop == 'TeamColor' then
									Box.Color3 = (v.TeamColor == Player.TeamColor and Color3.new(0,1,0)) or Color3.new(1,0,0)
								end
							end)

							local OnRemove; OnRemove = Player.CharacterRemoving:connect(function()
								Box:Destroy()
								VChange:disconnect()
								if not ShowInvisibleParts then
									OnChange:disconnect()
								end;
								PlayerChange:disconnect()
								OnRemove:disconnect()
							end)
						end
					end
				...
				function Track(v)
					...
					local vHum = v and v.Character and v.Character:FindFirstChild'Humanoid'
					local vHRP = vHum and vHum.Parent:FindFirstChild'HumanoidRootPart'

					if vHRP and vHum.Health > 0 then
						local Head = v.Character:WaitForChild('Head', 10)
						if Head then
							TrackHum(v, vHRP)
						else
					...
					local OnAdded = v.CharacterAdded:connect(function(char)
						local vHRP2 = char:WaitForChild'HumanoidRootPart'
					...
					local ChildRemove; ChildRemove = Players.PlayerRemoving:connect(function(Plr)
						if Plr == v then
							ChildRemove:disconnect();
							OnAdded:disconnect();
						end
					end)
				end
		...
		Chams:Set(Settings.Chams)

		local CrossHair = Tab:AddSwitch("CrossHair", function(bool)
			if bool then
				Settings.CrossHair = true
				Save()
				local Mouse = game.Players.LocalPlayer:GetMouse()
				local CrossHairID = "http://www.roblox.com/asset/?id=205898768"
		...
		CrossHair:Set(Settings.CrossHair)

		

		local infammo = Tab2:AddSwitch("Infinite Ammo", function(bool)
			if bool then
				Settings.InfiniteAmmo = true
				Save()
		...
		infammo:Set(Settings.InfiniteAmmo)
		local NoRecoil = Tab2:AddSwitch("No Recoil", function(bool)
			if bool then
		...
		local NoReload = Tab2:AddSwitch("No Reload", function(bool)
			if bool then
				Settings.NoReload = true
				Save()
				local haxx = getsenv(game.Players.LocalPlayer.PlayerGui.GUI.Client)
				haxx.reloadtime = 0

			else
				Settings.NoReload = false
				Save()
				local haxx = getsenv(game.Players.LocalPlayer.PlayerGui.GUI.Client)
				haxx.reloadtime = 1
		...
		NoReload:Set(Settings.NoReload)

		local NoSpread = Tab2:AddSwitch("No Spread", function(bool)
			if bool then
		...
		NoSpread:Set(Settings.NoSpread)

		local SilentAim = Tab3:AddSwitch("Silent Aim", function(bool)
			if bool then
				Settings.SilentAimt = true
				Save()
				...
				Settings.SilentAimt = false
				Save()

			end
		end)

		SilentAim:Set(Settings.SilentAimt)

		local Wallbang = Tab3:AddSwitch("Wallbang", function(bool)
			if bool then
		...
		Wallbang:Set(Settings.Wallbang)

		local KillAll = Tab3:AddSwitch("Kill All", function(bool)
			if bool then
				Settings.KllALL = true
				Save()
				local a
				local b
							...
							if v and v.Character and v.Character:FindFirstChild("Head") then
								...
								for i = 1,5 do
									game.ReplicatedStorage.Events.HitPart:FireServer(v.Character.Head,
										v.Character.Head.Position + Vector3.new(math.random(), math.random(), math.random(), math.random()),
										Gun.Name,
										Crit and 2 or 1,
										Distance,
										Backstab,
				...
				Settings.KllALL = false
				Save()
			end
		end)

		KillAll:Set(Settings.KllALL)

		local KillAura = Tab3:AddSwitch("Kill Aura", function(bool)
			if bool then
		...
		KillAura:Set(Settings.KillAura)

		local AutoFarm = Tab3:AddSwitch("AutoFarm", function(bool)
			if bool then 
				Settings.AutoFarm = true
				Save()
				loadstring(game:HttpGet("https://pastebin.com/raw/PatA14gu"))()
			else
		...
		AutoFarm:Set(Settings.AutoFarm)

		local AntiAim = Tab4:AddSwitch("Anti Aim", function(bool)
			if bool then
				Settings.AntiAim = true
				Save()
		...
		AntiAim:Set(Settings.AntiAim)

		local InfiniteJump = Tab4:AddSwitch("Infinite Jump", function(bool)
			if bool then
		...
		InfiniteJump:Set(Settings.InfiniteJump)

		local InstantRespawn = Tab4:AddSwitch("Instant Respawn", function(bool)
			if bool then
		...
		InstantRespawn:Set(Settings.InstantRespawn)

		local NoFallDamage = Tab4:AddSwitch("No Fall Damage", function(bool)
			if bool then
				Settings.NoFallDamage = true
				Save()
				local haxx = getsenv(game.Players.LocalPlayer.PlayerGui.GUI.Client)
				haxx.inducefalldamage = function(...) return nil end
			else
		...
		NoFallDamage:Set(Settings.NoFallDamage)

		local HitBoxExpander = Tab4:AddSwitch("HitBox Expander", function(bool)
			if bool then
				Settings.ExspandHitBox = true
				Save()
				hitboxSize = 13 
				visiblity = 10 
				...
				Settings.ExspandHitBox = false
				Save()
			end
		end)

		HitBoxExpander:Set(Settings.ExspandHitBox)

		local BHop = Tab4:AddSwitch("BHop", function(bool)
			if bool then
				Settings.BHop = true
				Save()
				...
				rs.RenderStepped:Connect(function()
					game:GetService("Players").LocalPlayer.Character:FindFirstChildofClass("Humanoid"):ChanageState("Jumping")
				end)    
			else
				Settings.BHop = false
				local rs = game:GetService("RunService")
				...
				local Character = LocalPlayer.Character
				rs.RenderStepped:Connect(function()
					game:GetService("Players").LocalPlayer.Character:FindFirstChildofClass("Humanoid"):ChanageState("Landed")
				end)
				Save()
			end
		end)

		BHop:Set(Settings.BHop)
		
		Tab5:AddLabel("0xSingularity for making the Imgui Ui Library")
		Tab5:AddLabel("Blissful for ESP and chams")
		Tab5:AddLabel("Bill for BHop and being Owner")
