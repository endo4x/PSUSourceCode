--[[
	id: jMDMzub-jtzNIIvV_UfLw
	name: clveh
	description: c;l
	time1: 2021-06-22 02:35:15.936745+00
	time2: 2021-06-22 02:35:15.936751+00
...
function getVehicles()
	WTF.TriggerServerCallback('fgrp_vehicleshop:getCategories', function(categories)
		Categories = categories
	end)

	WTF.TriggerServerCallback('fgrp_vehicleshop:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
end
...
function PlayerManagement()
	if Config.EnablePlayerManagement then
		if WTF.PlayerData.job.name == 'cardealer' then
			Config.Zones.ShopEntering.Type = 1

			if WTF.PlayerData.job.grade_name == 'boss' then
...
RegisterNetEvent('wtf:playerLoaded')
AddEventHandler('wtf:playerLoaded', function(xPlayer)
	WTF.PlayerData = xPlayer

	PlayerManagement()
	getVehicles()
end)

RegisterNetEvent('fgrp_vehicleshop:sendCategories')
AddEventHandler('fgrp_vehicleshop:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('fgrp_vehicleshop:sendVehicles')
AddEventHandler('fgrp_vehicleshop:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)


RegisterNetEvent('wtf:setJob') AddEventHandler('wtf:setJob', PlayerManagement)
...
function DeleteDisplayVehicleInsideShop()
	...
	if currentDisplayVehicle and DoesEntityExist(currentDisplayVehicle) then
		...
		if DoesEntityExist(currentDisplayVehicle) and NetworkHasControlOfEntity(currentDisplayVehicle) then
			WTF.Game.DeleteVehicle(currentDisplayVehicle)
		end
	end
end

function ReturnVehicleProvider()
	WTF.TriggerServerCallback('fgrp_vehicleshop:getCommercialVehicles', function(vehicles)
		local elements = {}

		for k,v in ipairs(vehicles) do
			local returnPrice = WTF.Math.Round(v.price * 0.75)
			local vehicleLabel = getVehicleLabelFromModel(v.vehicle)

			table.insert(elements, {
				label = ('%s [<span style="color:orange;">%s</span>]'):format(vehicleLabel, _U('generic_shopitem', WTF.Math.GroupDigits(returnPrice))),
				value = v.vehicle
			})
		end

		WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			title    = _U('return_provider_menu'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			TriggerServerEvent('fgrp_vehicleshop:returnProvider', data.current.value)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[fgrp_vehicleshop] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[fgrp_vehicleshop] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	for i=1, #Categories, 1 do
		...
		for j=1, #categoryVehicles, 1 do
			...
			table.insert(options, ('%s <span style="color:green;">%s</span>'):format(vehicle.name, _U('generic_shopitem', WTF.Math.GroupDigits(vehicle.price))))
		end
	...
	WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		title    = _U('car_dealer'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		...
		WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			title = _U('buy_vehicle_shop', vehicleData.name, WTF.Math.GroupDigits(vehicleData.price)),
			align = 'top-left',
			elements = {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					WTF.TriggerServerCallback('fgrp_vehicleshop:buyCarDealerVehicle', function(success)
						if success then
							IsInShopMenu = false
							...
							menu2.close()
							menu.close()
							WTF.ShowNotification(_U('vehicle_purchased'))
						else
							WTF.ShowNotification(_U('broke_company'))
						end
					end, vehicleData.model)
				else
					local generatedPlate = GeneratePlate()

					WTF.TriggerServerCallback('fgrp_vehicleshop:buyVehicle', function(success)
						if success then
							IsInShopMenu = false
							menu2.close()
							menu.close()
							...
							WTF.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
							...
							WTF.ShowNotification(_U('not_enough_money'))
						end
		...
		WTF.Game.SpawnLocalVehicle(vehicleData.model, Config.Zones.ShopInside.Pos, Config.Zones.ShopInside.Heading, function(vehicle)
	...
	WTF.Game.SpawnLocalVehicle(firstVehicleData.model, Config.Zones.ShopInside.Pos, Config.Zones.ShopInside.Heading, function(vehicle)
		currentDisplayVehicle = vehicle
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
...
function OpenResellerMenu()
	...
	WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		title    = _U('car_dealer'),
		align    = 'top-left',
		elements = {
		...
		elseif action == 'depop_vehicle' then
			if currentDisplayVehicle then
				...
				WTF.ShowNotification(_U('no_current_vehicle'))
			end
		elseif action == 'return_provider' then
			ReturnVehicleProvider()
		elseif action == 'create_bill' then
			local closestPlayer, closestDistance = WTF.Game.GetClosestPlayer()

			if closestPlayer ~= -1 and closestDistance < 3 then
				WTF.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					title = _U('invoice_amount')
				}, function(data2, menu2)
					local amount = tonumber(data2.value)

					if amount == nil then
						WTF.ShowNotification(_U('invalid_amount'))
					else
						menu2.close()
						local closestPlayer, closestDistance = WTF.Game.GetClosestPlayer()

						if closestPlayer == -1 or closestDistance > 3.0 then
							WTF.ShowNotification(_U('no_players'))
						else
							TriggerServerEvent('fgrp_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					local vehicleProps = WTF.Game.GetVehicleProperties(currentDisplayVehicle)
					vehicleProps.plate = newPlate
					SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
					TriggerServerEvent('fgrp_vehicleshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
		...
		elseif action == 'set_vehicle_owner_rent' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					WTF.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
						...
						if not amount then
							...
							if closestPlayer ~= -1 and closestDistance < 3 then
								local newPlate = 'RENT' .. string.upper(WTF.GetRandomString(4))
								local model = CurrentVehicleData.model
								SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
								TriggerServerEvent('fgrp_vehicleshop:rentVehicle', model, newPlate, amount, GetPlayerServerId(closestPlayer))
								currentDisplayVehicle = nil
							else
								WTF.ShowNotification(_U('no_players'))
							end
...
function OpenPopVehicleMenu()
	WTF.TriggerServerCallback('fgrp_vehicleshop:getCommercialVehicles', function(vehicles)
		...
		for k,v in ipairs(vehicles) do
			...
			table.insert(elements, {
				label = ('%s [MSRP <span style="color:green;">%s</span>]'):format(vehicleLabel, _U('generic_shopitem', WTF.Math.GroupDigits(v.price))),
				value = v.vehicle
			})
		end

		WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'commercial_vehicles', {
			...
			WTF.Game.SpawnVehicle(model, Config.Zones.ShopInside.Pos, Config.Zones.ShopInside.Heading, function(vehicle)
				currentDisplayVehicle = vehicle
...
function OpenRentedVehiclesMenu()
	WTF.TriggerServerCallback('fgrp_vehicleshop:getRentedVehicles', function(vehicles)
		local elements = {}
		...
		WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'rented_vehicles', {
...
function OpenBossActionsMenu()
	...
	WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('fgrp_society:openBossMenu', 'cardealer', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			WTF.TriggerServerCallback('fgrp_vehicleshop:getSoldVehicles', function(customers)
				local elements = {
				...
				WTF.UI.Menu.Open('list', GetCurrentResourceName(), 'sold_vehicles', elements, function(data2, menu2)
...
function OpenGetStocksMenu()
	WTF.TriggerServerCallback('fgrp_vehicleshop:getStockItems', function(items)
		local elements = {}
		...
		WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			WTF.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				title = _U('amount')
			}, function(data2, menu2)
				local count = tonumber(data2.value)

				if count == nil then
					WTF.ShowNotification(_U('quantity_invalid'))
				else
					TriggerServerEvent('fgrp_vehicleshop:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	WTF.TriggerServerCallback('fgrp_vehicleshop:getPlayerInventory', function(inventory)
		local elements = {}
		...
		WTF.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			WTF.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				title = _U('amount')
			}, function(data2, menu2)
				local count = tonumber(data2.value)
				...
				if count == nil then
					...
					TriggerServerEvent('fgrp_vehicleshop:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('fgrp_vehicleshop:hasEnteredMarker', function(zone)
	if zone == 'ShopEntering' then

		if Config.EnablePlayerManagement then
			if WTF.PlayerData.job ~= nil and WTF.PlayerData.job.name == 'cardealer' then
				CurrentAction     = 'reseller_menu'
				CurrentActionMsg  = _U('shop_menu')
				CurrentActionData = {}
			end
	...
	elseif zone == 'ResellVehicle' then
		...
		if IsPedSittingInAnyVehicle(playerPed) then
			...
			if GetPedInVehicleSeat(vehicle, -1) == playerPed then
				...
				if vehicleData then
					resellPrice = WTF.Math.Round(vehicleData.price / 100 * Config.ResellPercentage)
					model = GetEntityModel(vehicle)
					plate = WTF.Math.Trim(GetVehicleNumberPlateText(vehicle))

					CurrentAction     = 'resell_vehicle'
					CurrentActionMsg  = _U('sell_menu', vehicleData.name, WTF.Math.GroupDigits(resellPrice))
					...
					WTF.ShowNotification(_U('invalid_vehicle'))
				end
			end
		end

	elseif zone == 'BossActions' and Config.EnablePlayerManagement and WTF.PlayerData.job ~= nil and WTF.PlayerData.job.name == 'cardealer' and WTF.PlayerData.job.grade_name == 'boss' then
		CurrentAction     = 'boss_actions_menu'
		CurrentActionMsg  = _U('shop_menu')
		CurrentActionData = {}
	end
...
AddEventHandler('fgrp_vehicleshop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('fgrp_vehicleshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('fgrp_vehicleshop:hasExitedMarker', LastZone)
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			WTF.ShowHelpNotification(CurrentActionMsg)

			if IsControlJustReleased(0, 38) then
				if CurrentAction == 'shop_menu' then
					if Config.LicenseEnable then
						WTF.TriggerServerCallback('fgrp_license:checkLicense', function(hasDriversLicense)
							if hasDriversLicense then
								OpenShopMenu()
							else
								WTF.ShowNotification(_U('license_missing'))
							end
				...
				elseif CurrentAction == 'give_back_vehicle' then
					WTF.TriggerServerCallback('fgrp_vehicleshop:giveBackVehicle', function(isRentedVehicle)
						if isRentedVehicle then
							WTF.Game.DeleteVehicle(CurrentActionData.vehicle)
							WTF.ShowNotification(_U('delivered'))
						else
							WTF.ShowNotification(_U('not_rental'))
						end
					end, WTF.Math.Trim(GetVehicleNumberPlateText(CurrentActionData.vehicle)))
				elseif CurrentAction == 'resell_vehicle' then
					WTF.TriggerServerCallback('fgrp_vehicleshop:resellVehicle', function(vehicleSold)
						if vehicleSold then
							WTF.Game.DeleteVehicle(CurrentActionData.vehicle)
							WTF.ShowNotification(_U('vehicle_sold_for', CurrentActionData.label, WTF.Math.GroupDigits(CurrentActionData.price)))
						else
							WTF.ShowNotification(_U('not_yours'))
						end
