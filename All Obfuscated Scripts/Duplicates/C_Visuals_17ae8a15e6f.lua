--[[
	id: 6gL0Tsen2ZH9bpMZZVtmm
	name: C_Visuals2
	description: C_Visuals2
	time1: 2021-07-13 18:48:05.666065+00
	time2: 2021-07-13 18:48:05.666066+00
...
-----UI ADDITIONS-----
----------------------
local label = ui.new_label("Lua", "b", '                    =3.Nade Esp=')

local ui_additions = {
    nade_esp = {
        ui_enabled = api.ui.new_multiselect("lua", "b", "Nade ESP", "Smoke", "Molotov"),
        ui_settings = api.ui.new_combobox("lua", "b", "Nade ESP settings", "General", "Smoke", "Molotov"),
        settings = {
            molotov = {
                radius_color_label = api.ui.new_label("lua", "b", "Radius color"),
                radius_color = api.ui.new_color_picker("lua", "b", "radius_color", 255, 0, 0, 255),
                background_color_label = api.ui.new_label("lua", "b", "Background"),
                background_color = api.ui.new_color_picker("lua", "b", "background_color", 0, 0, 0, 200),
                icon_color_label = api.ui.new_label("lua", "b", "Icon"),
                icon_color = api.ui.new_color_picker("lua", "b", "icon_color", 255, 255, 255, 255),
                indicator_color_label = api.ui.new_label("lua", "b", "Indicator"),
                indicator_color = api.ui.new_color_picker("lua", "b", "indicator_color", 255, 255, 255, 255),
                hvhmode = api.ui.new_checkbox("lua", "b", "HvH-mode"),
                hvh_safe_color_label = api.ui.new_label("lua", "b", "  safe"),
                hvh_safe_color = api.ui.new_color_picker("lua", "b", "hvh_icon_safe_color", 0, 255, 0, 255),
                hvh_unsafe_color_label = api.ui.new_label("lua", "b", "  unsafe"),
                hvh_unsafe_color = api.ui.new_color_picker("lua", "b", "hvh_icon_unsafe_color", 255, 0, 0, 255)
            },
            smoke = {
                radius_color_label = api.ui.new_label("lua", "b", "Radius color"),
                radius_color = api.ui.new_color_picker("lua", "b", "smoke_radius_color", 0, 0, 255, 255),
                background_color_label = api.ui.new_label("lua", "b", "Background"),
                background_color = api.ui.new_color_picker("lua", "b", "smoke_background_color", 0, 0, 0, 200),
                icon_color_label = api.ui.new_label("lua", "b", "Icon"),
                icon_color = api.ui.new_color_picker("lua", "b", "smoke_icon_color", 255, 255, 255, 255),
                indicator_color_label = api.ui.new_label("lua", "b", "Indicator"),
                indicator_color = api.ui.new_color_picker("lua", "b", "smoke_indicator_color", 255, 255, 255, 255)
            },
            general = {
                fade_out = api.ui.new_checkbox("lua", "b", "Fade out"),
                circle_radius = api.ui.new_slider("lua", "b", "Indicator radius", 10, 30, 20, true, "px", 1, nil),
                outline_thickness = api.ui.new_slider("lua", "b", "Outline thickness", 1, 5, 2, true, "px", 1, nil),
                icon_size = api.ui.new_slider("lua", "b", "Icon size", 10, 50, 20, true, "px", 1, nil)
            }
        }
    }
}
...
local function HIDE_MEAN()
    if ui.get(hide_mean) then
        ...
        ui.set_visible(ui_additions.nade_esp.ui_enabled,false)
        ui.set_visible(ui_additions.nade_esp.settings.general.fade_out,false)
        ui.set_visible(ui_additions.nade_esp.settings.general.circle_radius,false)
        ui.set_visible(ui_additions.nade_esp.settings.general.outline_thickness,false)
        ui.set_visible(ui_additions.nade_esp.settings.general.icon_size,false)

    else
        
        ui.set_visible(ui_additions.nade_esp.ui_enabled,true)
        ui.set_visible(ui_additions.nade_esp.settings.general.fade_out,true)
        ui.set_visible(ui_additions.nade_esp.settings.general.circle_radius,true)
        ui.set_visible(ui_additions.nade_esp.settings.general.outline_thickness,true)
        ui.set_visible(ui_additions.nade_esp.settings.general.icon_size,true)
...
local label = ui.new_label("Lua", "b", '                    =4.Nade radius=')
...
local function HIDE_MEAN()
    if ui_get(hide_mean) then
        ui_set_visible(grenade_timer_reference,false)
        ui_set_visible(molotov_team_reference,false)
        ui_set_visible(smoke_radius_reference,false)
        ui_set_visible(smoke_radius_color_reference,false)
        ui_set_visible(molotov_radius_reference,false)
        ui_set_visible(molotov_radius_color_reference,false)
    
       
    else
        ui_set_visible(grenade_timer_reference,true)
        ui_set_visible(molotov_team_reference,true)
        ui_set_visible(smoke_radius_reference,true)
        ui_set_visible(smoke_radius_color_reference,true)
        ui_set_visible(molotov_radius_reference,true)
        ui_set_visible(molotov_radius_color_reference,true)
    end
end
