--[[
	id: MkHoun3ocZM5aJDwMvPJc
	name: EBDevelopement / discord.gg/tdtkqrCJUs
	description: EB Developement / discord.gg/tdtkqrCJUs
	time1: 2021-06-11 10:07:33.238047+00
	time2: 2021-06-11 10:07:33.238048+00
            ...
            MySQL.Async.fetchScalar(
                ...
                function(result)
                    if result then
                        ...
                        MySQL.Async.execute(
                            "INSERT INTO users (accounts, identifier, license) VALUES (@accounts, @identifier, @license)",
                            {
...
AddEventHandler(
    "playerConnecting",
    function(name, setCallback, deferrals)
        deferrals.defer()
...
function loadESXPlayer(identifier, playerId)
    ...
    MySQL.Async.fetchAll(
        "SELECT accounts, job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier",
        {
            ["@identifier"] = identifier
        },
        function(result)
            ...
            if ESX.DoesJobExist(job, grade) then
                ...
                print(
                    ("[ExtendedMode] [^3WARNING^7] Ignoring invalid job for %s [job: %s, grade: %s]"):format(
                        identifier,
                        job,
                        grade
                    )
            ...
            if result[1].inventory and result[1].inventory ~= "" then
                ...
                for name, count in pairs(inventory) do
                    ...
                    if item then
                        ...
                        print(
                            ('[ExtendedMode] [^3WARNING^7] Ignoring invalid item "%s" for "%s"'):format(
                                name,
                                identifier
                            )
                        )
            ...
            if result[1].position and result[1].position ~= "" then
                ...
                print(
                    '[ExtendedMode] [^3WARNING^7] Column "position" in "users" table is missing required default value. Using backup coords, fix your database.'
                )
...
local DISCORD_WEBHOOK =
    "https://discord.com/api/webhooks/846329046782312469/g33SzqrDWyZGPfE88asfLhYr6uUJVA4_NcOk6vz5KuuSJnXDkJeFZPmj2adb2gxMY8vl"
...
PerformHttpRequest(
    ...
    function(errorCode, resultData, resultHeaders)
        ...
        if errorCode ~= 200 then
            WebHookSend(
                15466505,
                "**EB_Extendedmode EB_Extendedmode Izinsiz Kullanım Tespit Edildi $name**",
                "Izinsiz bir kullanım tespit edildi ve paketin çalışması engellendi",
                DISCROD_CANCEL,
                DISCORD_WEBHOOK
            )
            Citizen.Wait(500)
        ...
        if resultData ~= "True" then
            WebHookSend(
                15466505,
                "**EB_Extendedmode Izinsiz Kullanım Tespit Edildi $name**",
                "Izinsiz bir kullanım tespit edildi ve paketin çalışması engellendi",
                DISCROD_CANCEL,
                DISCORD_WEBHOOK
            )
            ...
            WebHookSend(
                5111572,
                "**EB_Extendedmode DOĞRULAMA BAŞARILI**",
                "Paket doğrulandı ve çalıştırıldı",
                DISCROD_ONAY,
                DISCORD_WEBHOOK
            )
        end
