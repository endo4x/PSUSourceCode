--[[
	id: SZMrB_OoSqBWrQqCCRNwM
	name: party
	description: lol
	time1: 2021-07-03 16:55:38.253872+00
	time2: 2021-07-03 16:55:38.253872+00
	uploader: J6trGhoW4jGKqhhgzpaV2m7Rgab_gvD-dDYhWgJC
	uploadersession: kdkb0Iu2ACJtDJjlYueQQCKktiEwqK
	flag: f
--]]
...
if ( Feature.get('Panorama Libraries') ) then
	...
	CPPanoramaMainMenu = panorama.loadstring([[
		...
		*/
		...
		PartyChatCommands.push({
			title: 'Nigger (!\u{200B}nigger)',
			cmds: ['nigger'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`${sender} | ${steamid} is a nigger`)
			}
		});
		PartyChatCommands.push({
			title: 'Group (!\u{200B}group)',
			cmds: ['group'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`${sender} https://steamcommunity.com/groups/rageaa join or gay`)
			}
		});
		PartyChatCommands.push({
			title: 'Playlist (!\u{200B}playlist)',
			cmds: ['playlist'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`https://www.youtube.com/playlist?list=PLwtKAcCwe0jX6g294x5HtUzHVI9avUdyJ`)
			}
		});
		PartyChatCommands.push({
			title: 'skeetless (!\u{200B}skeetless)',
			cmds: ['skeetless'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`${sender}, is a skeetless noname`)
			}
		});
		PartyChatCommands.push({
			title: 'Nya (!\u{200B}nya)',
			cmds: ['nya'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`${sender} , uwu nyaa~~ *nuzzles*`)
			}
		});
		PartyChatCommands.push({
			title: 'Tranny (!\u{200B}tranny)',
			cmds: ['tranny'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`${sender} , wants to know who a tranny is : https://steamcommunity.com/profiles/76561198367725056`)
			}
		});
		PartyChatCommands.push({
			title: 'Cheat (!\u{200B}Cheat)',
			cmds: ['cheat'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`${sender} , i'm using gamesense.pub`)
			}
		});
		PartyChatCommands.push({
			title: 'Novo (!\u{200B}novo)',
			cmds: ['novo'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`${sender}, novo sucks ass get good get gamesense :)`)
			}
		});
		PartyChatCommands.push({
			title: 'Cute (!\u{200B}cute)',
			cmds: ['cute'],
			exec: (cmd, args, sender, steamid) => {
				if ( Utilities.IsBlacklisted(steamid) ) return;
				Utilities.SayParty(`${sender} , is a cute person uwu`)
			}
		});
		PartyChatCommands.push({
