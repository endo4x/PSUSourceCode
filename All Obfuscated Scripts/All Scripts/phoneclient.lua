--[[
	id: QfgfgwmsuVQZGnkDDFTtt
	name: phone-client
	description: phone-client
	time1: 2021-06-07 23:25:48.715542+00
	time2: 2021-06-07 23:25:48.715542+00
...
RegisterNUICallback("GetCurrentArrests", function(data, cb)
    ESX.TriggerServerCallback('cash-telephone:server:GetCurrentArrests', function(arrests)
        print(ESX.DumpTable(arrests))
        cb(arrests)
    end)
end)
...
RegisterNUICallback("UpdateAvailableStatus", function(data, cb)
    TriggerServerEvent("qb-phone:UpdateAvailableStatus", data.available)
    cb("ok")
end)
...
RegisterCommand("phonefix", function()
 ...
 local phone = false
 function OpenPhone()
    ESX.TriggerServerCallback('qb-phone:checkServerLogin', function(login)
        if login then
         ESX.TriggerServerCallback('qb-phone:server:HasPhone', function(HasPhone)
             if HasPhone then
...
RegisterNUICallback('GetInvoices', function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetInvoices',function(data)
...
RegisterNUICallback('CallContact', function(data, cb)
    ESX.TriggerServerCallback('qb-phone:server:GetCallState', function(CanCall, IsOnline, IsAvailable)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
CancelCall = function()
    TriggerServerEvent('qb-phone:server:CancelCall', PhoneData.CallData, GetPlayerServerId(PlayerId()))
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('qb-phone:server:AnswerCall', PhoneData.CallData, GetPlayerServerId(PlayerId()))
        -- if Config.Tokovoip then
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    ESX.TriggerServerCallback('cash-telephone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)

RegisterNUICallback('GetCurrentpolices', function(data, cb)
    ESX.TriggerServerCallback('cash-telephone:server:GetCurrentpolices', function(lawyers)
        cb(lawyers)
    end)
end)

RegisterNUICallback("GetCurrentDrivers", function(data, cb)
    ESX.TriggerServerCallback('cash-telephone:server:GetCurrentDrivers', function(drivers)
        cb(drivers)
    end)
end)

RegisterNUICallback("GetCurrentMecano", function(data, cb)
    ESX.TriggerServerCallback('cash-telephone:server:GetCurrentMecano', function(drivers)
        cb(drivers)
    end)
end)

RegisterNUICallback("GetCurrentDoctor", function(data, cb)
    ESX.TriggerServerCallback('cash-telephone:server:GetCurrentDoctor', function(drivers)
        cb(drivers)
    end)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for i = 1, #PhoneData.Alarms do
            ...
            if PhoneData.Alarms[i] == tostring((d+3)..":"..e) then
...
AddEventHandler('qb-phone:PlayAlarmSound', function()
    exports.mythic_notify:SendAlert('inform', 'Alarm çalıyor.')
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 3.0, "radaralarm", 0.2)
end)