--[[
	id: LahF8MTL5BiMXpjChagKn
	name: ferrari
	description: .
	time1: 2021-07-24 09:25:01.892064+00
	time2: 2021-07-24 09:25:01.892065+00
	uploader: OQvGogg7eCwAMx9RIz4zyn0nw1HWl3O3tsLL_ePW
	uploadersession: rD2an5T9PxwExQ1CWABZPApfD2BGAD
	flag: f
--]]
...
local title = ui.new_label("LUA", "B", "========= FERRARI =========")
local modeswitch = ui.new_combobox("LUA", "B", "Mode", {"-" ,"Anti-Aim", "Exploits", "Misc"})

local aatable = {
    legitaa = ui.new_checkbox("LUA", "B", "Legit AA on E"),
    lowdeltakey =  ui.new_hotkey("LUA", "B", "Low Delta on Key"),
    manualback = ui.new_hotkey("LUA", "B", "Back"),
    manualright = ui.new_hotkey("LUA", "B", "Right"),
    manualleft = ui.new_hotkey("LUA", "B", "Left")
}

local misctable = {
    idealtickkey = ui.new_checkbox("LUA", "B", "Ideal Tick"),
    aa_legs = ui.new_checkbox("LUA", "B", "Leg Breaker"),
    ferrari_resolver = ui.new_checkbox("LUA", "B", "Resolver")
}

local exploitstable = {
    fasterdt = ui.new_checkbox("LUA", "B", "Faster DT"),
}
...
local function handleGUI()
    ...
    if ui.get(modeswitch) == "Misc" then
        ...
        ui.set_visible(misctable.ferrari_resolver, true)
    else
        ui.set_visible(misctable.idealtickkey, false)
        ui.set_visible(misctable.aa_legs, false)
        ui.set_visible(misctable.ferrari_resolver, false)
    end
end
...
local function legfucker()
    if ui.get(misctable.aa_legs) then
...
client.color_log(140, 255, 234, "[ferrari V1] Loaded. [24/07/21]")
...
client.set_event_callback("paint", function()
    if ui.get(misctable.ferrari_resolver) then
