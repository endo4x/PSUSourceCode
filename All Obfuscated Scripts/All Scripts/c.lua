--[[
	id: c1QfUvtow69X-G-k8do8E
	name: 卡比獸c
	description: 卡比獸c
	time1: 2021-05-01 00:56:31.286589+00
	time2: 2021-05-01 00:56:31.28659+00
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			local setpos = PedPosition.x..','..PedPosition.y..','..PedPosition.z
			if DeathReason == '自殺' or DeathReason == '死亡' then
				TriggerServerEvent('DiscordBot:playerDied', GetPlayerName(PlayerId()) .. ' ' .. DeathReason, Weapon,'no',setpos)
			else
				TriggerServerEvent('DiscordBot:playerDied', GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. GetPlayerName(PlayerId()) .. '.', Weapon,GetPlayerServerId(Killer),setpos)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
