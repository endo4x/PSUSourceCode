--[[
	id: hzp5Pf-qrpJbNsSsfK_hO
	name: sp_vehicleshop
	description: lavoro concessionario
	time1: 2021-04-24 18:08:24.825277+00
	time2: 2021-04-24 18:08:24.825278+00
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	xPlayer.PlayerData = xPlayer

	if Config.EnablePlayerManagement then
		if xPlayer.PlayerData.job.name == 'cardealer' then
			Config.Zones.ShopEntering.Type = 1

			if xPlayer.PlayerData.job.grade_name == 'boss' then
				Config.Zones.BossActions.Type = 1
			end
...
RegisterNetEvent('sp_vehicleshop:sendCategories')
AddEventHandler('sp_vehicleshop:sendCategories', function (categories)
	Categories = categories
end)

RegisterNetEvent('sp_vehicleshop:sendVehicles')
AddEventHandler('sp_vehicleshop:sendVehicles', function (vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('sp_vehicleshop:getCommercialVehicles', function (vehicles)
		local elements = {}
		local returnPrice
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('sp_vehicleshop:returnProvider', data.current.value)
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('sp_vehicleshop: il veicolo "%s" non esiste'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('sp_vehicleshop:buyVehicleSociety', function(hasEnoughMoney)
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('sp_vehicleshop:buyVehicle', function(hasEnoughMoney)
									if hasEnoughMoney then
										IsInShopMenu = false
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											if Config.EnableOwnedVehicles then
												TriggerServerEvent('sp_vehicleshop:setVehicleOwned', vehicleProps, vehicleData)
								...
								ESX.TriggerServerCallback('sp_vehicleshop:buyVehicleSociety', function (hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											vehicleProps.plate = newPlate
											SetVehicleNumberPlateText(vehicle, newPlate)
											TriggerServerEvent('sp_vehicleshop:setVehicleOwnedSociety', playerData.job.name, vehicleProps)
											if Config.EnableJobLogs == true then
						...
						ESX.TriggerServerCallback('sp_vehicleshop:buyVehicle', function (hasEnoughMoney)
							if hasEnoughMoney then
								...
								ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
									...
									if Config.EnableOwnedVehicles then
										TriggerServerEvent('sp_vehicleshop:setVehicleOwned', vehicleProps)
										if Config.EnableJobLogs == true then
											TriggerServerEvent('esx_joblogs:AddInLog', 'vehicleshop', 'selfcarbuy', GetPlayerName(PlayerId()), vehicleProps.plate, VehicleName)
										end
									end
...
function OpenResellerMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		...
		elements = {
			...
			{label = '📣 Aperto',                    value = 'concess_ouvert'},
			{label = '📢 Chiuso',                    value = 'concess_ferme'},
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						if Config.EnableJobLogs == true then
							TriggerServerEvent('esx_joblogs:AddInLog', "vehicleshop", "sendBill", GetPlayerName(PlayerId()), GetPlayerName(closestPlayer), _U('car_dealer'), data2.value)
						end
					end
				end
			end, function (data2, menu2)
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				TriggerServerEvent('sp_vehicleshop:sellVehicle', model)
				TriggerServerEvent('sp_vehicleshop:addToList', GetPlayerServerId(closestPlayer), model, newPlate)

				if Config.EnableOwnedVehicles then
					TriggerServerEvent('sp_vehicleshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps)
					if Config.EnableJobLogs == true then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('esx:getOtherPlayerData', function (xPlayer)
					...
					if Config.EnableSocietyOwnedVehicles then
						TriggerServerEvent('sp_vehicleshop:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps)
						if Config.EnableJobLogs == true then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(LastVehicles[#LastVehicles], newPlate)
						TriggerServerEvent('sp_vehicleshop:rentVehicle', model, vehicleProps.plate, GetPlayerName(closestPlayer), CurrentVehicleData.price, amount, GetPlayerServerId(closestPlayer))
						if Config.EnableJobLogs == true then
						...
						TriggerServerEvent('sp_vehicleshop:setVehicleForAllPlayers', vehicleProps, Config.Zones.ShopInside.Pos.x, Config.Zones.ShopInside.Pos.y, Config.Zones.ShopInside.Pos.z, 5.0)
					end
				end
			end, function (data2, menu2)
				menu2.close()
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('sp_vehicleshop:getRentedVehicles', function (vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		elseif data.current.value == 'sold_vehicles' then
			...
			ESX.TriggerServerCallback('sp_vehicleshop:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('sp_vehicleshop:getStockItems', function (items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('sp_vehicleshop:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('sp_vehicleshop:getPlayerInventory', function (inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('sp_vehicleshop:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('sp_vehicleshop:hasEnteredMarker', function (zone)
...
AddEventHandler('sp_vehicleshop:hasExitedMarker', function (zone)
...
Citizen.CreateThread(function ()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('sp_vehicleshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('sp_vehicleshop:hasExitedMarker', LastZone)
		end
	end
end)
		...
		if CurrentAction == nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					ESX.TriggerServerCallback('sp_vehicleshop:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('sp_vehicleshop:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
