--[[
	id: bWhgMdyGjKbpSOihC7uHq
	name: racing - client
	description: lapraces
	time1: 2021-07-10 14:32:59.470038+00
	time2: 2021-07-10 14:32:59.470039+00
...
RegisterNetEvent('bl-lapraces:client:StartRaceEditor')
AddEventHandler('bl-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        QBCore.Functions.Notify('Your already making a race.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        QBCore.Functions.Notify('You have not placed a checkpoint ..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        QBCore.Functions.Notify('You cant go higher than 15')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        QBCore.Functions.Notify('You cant go lower than 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    QBCore.Functions.Notify('Press again on [9] to confirm', 'error', 5000)
                else
...
function SaveRace()
    ...
    TriggerServerEvent('bl-lapraces:server:SaveRace', CreatorData)
...
RegisterNetEvent('bl-lapraces:client:UpdateRaceRacerData')
AddEventHandler('bl-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('bl-lapraces:client:JoinRace')
AddEventHandler('bl-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('bl-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        QBCore.Functions.Notify('Your already in a race..', 'error')
    end
end)

RegisterNetEvent('bl-lapraces:client:LeaveRace')
AddEventHandler('bl-lapraces:client:LeaveRace', function(data)
...
RegisterNetEvent('bl-lapraces:client:RaceCountdown')
AddEventHandler('bl-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('bl-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('bl-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('bl-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('bl-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
...
RegisterNetEvent('bl-lapraces:client:PlayerFinishs')
AddEventHandler('bl-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
...
RegisterNetEvent('bl-lapraces:client:WaitingDistanceCheck')
AddEventHandler('bl-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            TriggerServerEvent('bl-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)
