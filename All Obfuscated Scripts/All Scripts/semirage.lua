--[[
	id: DfbigttLOMhmgZi8w9MtT
	name: semirage
	description: 123
	time1: 2021-07-23 06:04:39.023892+00
	time2: 2021-07-23 06:04:39.023893+00
	uploader: 4539
...
local on_aa_paint_event = function()
	...
	local w, h = client_screen_size()
	local x, y = w / 2, h / 2
	r1, g1, b1 = func_rgb_rainbowize(0.1, 1)
	renderer_indicator(r1, g1, b1, 255, "skyness config")
...
local mode = ui.new_combobox("AA", "Anti-aimbot angles", "Lower body yaw target", {"Sway", "Eye yaw", "Opposite"})
...
local function paint_ui()
    ui.set_visible(fake_yaw_limit, false)
end
...
local function setup_command(cmd)
    local mode_g = get(mode)

    if mode_g == "Eye yaw" then 
        return
    end
    ...
    if mode_g == "Sway" then
        stop_micromove(cmd)
    elseif mode_g == "Opposite"then
        set(fake_yaw_limit, 60)
