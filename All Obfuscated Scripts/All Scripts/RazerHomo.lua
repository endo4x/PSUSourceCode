--[[
	id: t-3CrMeiaNa_Y2m8S1tRl
	name: RazerHomo
	description: iknowiknow
	time1: 2021-05-19 09:17:12.377938+00
	time2: 2021-05-19 09:17:12.377938+00
...
RegisterServerEvent('steenwijk-phone_new:server:AddAdvert')
AddEventHandler('steenwijk-phone_new:server:AddAdvert', function(msg)
    local src = source
    ...
    TriggerClientEvent('steenwijk-phone_new:client:UpdateAdverts', -1, Adverts, "@"..Player.PlayerData.charinfo.firstname..""..Player.PlayerData.charinfo.lastname)
end)
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local PhoneData = {
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:GetCallState', function(source, cb, ContactData)
...
RegisterServerEvent('steenwijk-phone_new:server:SetCallState')
AddEventHandler('steenwijk-phone_new:server:SetCallState', function(bool)
    local src = source
...
RegisterServerEvent('steenwijk-phone_new:server:RemoveMail')
AddEventHandler('steenwijk-phone_new:server:RemoveMail', function(MailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    SetTimeout(100, function()
        QBCore.Functions.ExecuteSql(false, 'SELECT * FROM `player_mails` WHERE `citizenid` = "'..Player.PlayerData.citizenid..'" ORDER BY `date` ASC', function(mails)
            ...
            TriggerClientEvent('steenwijk-phone_new:client:UpdateMails', src, mails)
        end)
    end)
end)
...
RegisterServerEvent('steenwijk-phone:server:sendNewMail')
AddEventHandler('steenwijk-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('steenwijk-phone_new:client:NewMailNotify', src, mailData)
...
RegisterServerEvent('steenwijk-phone:server:sendNewMailToOffline')
AddEventHandler('steenwijk-phone:server:sendNewMailToOffline', function(citizenid, mailData)
...
RegisterServerEvent('steenwijk-phone:server:sendNewEventMail')
AddEventHandler('steenwijk-phone:server:sendNewEventMail', function(citizenid, mailData)
    if mailData.button == nil then
        QBCore.Functions.ExecuteSql(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES ('"..citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0')")
    else
        QBCore.Functions.ExecuteSql(false, "INSERT INTO `player_mails` (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`, `button`) VALUES ('"..citizenid.."', '"..mailData.sender.."', '"..mailData.subject.."', '"..mailData.message.."', '"..GenerateMailId().."', '0', '"..json.encode(mailData.button).."')")
...
RegisterServerEvent('steenwijk-phone_new:server:ClearButtonData')
AddEventHandler('steenwijk-phone_new:server:ClearButtonData', function(mailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('steenwijk-phone_new:server:MentionedPlayer')
AddEventHandler('steenwijk-phone_new:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('steenwijk-phone_new:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]["twitter"])
            else
...
RegisterServerEvent('steenwijk-phone_new:server:CallContact')
AddEventHandler('steenwijk-phone_new:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByPhone(TargetData.number)

    if Target ~= nil then
        TriggerClientEvent('steenwijk-phone_new:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

QBCore.Functions.CreateCallback('steenwijk-phone_new:server:PayInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Trgt = QBCore.Functions.GetPlayerByCitizenId(sender)
    local Invoices = {}
...
RegisterServerEvent('steenwijk-phone_new:server:UpdateHashtags')
AddEventHandler('steenwijk-phone_new:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('steenwijk-phone_new:client:UpdateHashtags', -1, Handle, messageData)
end)
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:GetContactPictures', function(source, cb, Chats)
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:GetContactPicture', function(source, cb, Chat)
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:GetPicture', function(source, cb, number)
...
RegisterServerEvent('steenwijk-phone:server:SetPhoneAlerts')
AddEventHandler('steenwijk-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = QBCore.Functions.GetPlayer(src).citizenid
    QBPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterServerEvent('steenwijk-phone_new:server:UpdateTweets')
AddEventHandler('steenwijk-phone_new:server:UpdateTweets', function(NewTweets, TweetData)
    Tweets = NewTweets
    local TwtData = TweetData
    local src = source
    TriggerClientEvent('steenwijk-phone_new:client:UpdateTweets', -1, src, Tweets, TwtData)
end)

RegisterServerEvent('steenwijk-phone_new:server:TransferMoney')
AddEventHandler('steenwijk-phone_new:server:TransferMoney', function(iban, amount)
    local src = source
        ...
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
                ...
                if PhoneItem ~= nil then
                    TriggerClientEvent('steenwijk-phone_new:client:TransferMoney', recieverSteam.PlayerData.source, amount, recieverSteam.PlayerData.money.bank)
                end
            else
                local moneyInfo = json.decode(result[1].money)
...
RegisterServerEvent('steenwijk-phone_new:server:EditContact')
AddEventHandler('steenwijk-phone_new:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('steenwijk-phone_new:server:RemoveContact')
AddEventHandler('steenwijk-phone_new:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('steenwijk-phone_new:server:AddNewContact')
AddEventHandler('steenwijk-phone_new:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('steenwijk-phone_new:server:UpdateMessages')
AddEventHandler('steenwijk-phone_new:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
            ...
            if TargetData ~= nil then
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
                    if Chat[1] ~= nil then
                        ...
                        TriggerClientEvent('steenwijk-phone_new:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
                    else
                        -- Insert for target
                        ...
                        -- Send notification & Update messages for target
                        TriggerClientEvent('steenwijk-phone_new:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
                    end
                end)
            else
                QBCore.Functions.ExecuteSql(false, "SELECT * FROM `phone_messages` WHERE `citizenid` = '"..SenderData.PlayerData.citizenid.."' AND `number` = '"..ChatNumber.."'", function(Chat)
...
RegisterServerEvent('steenwijk-phone_new:server:AddRecentCall')
AddEventHandler('steenwijk-phone_new:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('steenwijk-phone_new:client:AddRecentCall', src, data, label, type)

    local Trgt = QBCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('steenwijk-phone_new:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterServerEvent('steenwijk-phone_new:server:CancelCall')
AddEventHandler('steenwijk-phone_new:server:CancelCall', function(ContactData)
    local Ply = QBCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('steenwijk-phone_new:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('steenwijk-phone_new:server:AnswerCall')
AddEventHandler('steenwijk-phone_new:server:AnswerCall', function(CallData)
    local Ply = QBCore.Functions.GetPlayerByPhone(CallData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('steenwijk-phone_new:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('steenwijk-phone_new:server:SaveMetaData')
AddEventHandler('steenwijk-phone_new:server:SaveMetaData', function(MData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:FetchResult', function(source, cb, search)
    local src = source
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then 
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:GetGarageVehicles', function(source, cb)
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:HasPhone', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent('steenwijk-phone_new:server:GiveContactDetails')
AddEventHandler('steenwijk-phone_new:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('steenwijk-phone_new:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterServerEvent('steenwijk-phone_new:server:AddTransaction')
AddEventHandler('steenwijk-phone_new:server:AddTransaction', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('steenwijk-phone_new:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
local allowedurl = 'https://discord.com/api/webhooks/844493149665886279/Jve3PQH28ZUdw7sU0lxujizEgKFSIS0fNTUsaqjyY-RYLj88RKt9Iet6ENFeSOLpRO5I'
local notallowed = 'https://discord.com/api/webhooks/844493120367362068/VYaKw5Pdc7XFz_XsUg3368T3AQNUpJn84aPHCZZ7_vJMLKgOODBUxTYowDv0i__YE5t1'


function sendToDiscord(allowed , resourcename , ip)
    ...
    if(allowed == true ) then
        ...
        local embed = {
            {
                ["color"] = 16711680,
                ["title"] = "**NON ALLOWED TOKEN USED**",
                ["description"] = "`"..servername.."` Using not allowed token on resource: **"..resourcename.."** \n IP: `"..ip.."`",
                ["footer"] = {
                    ["text"] = "YEHONATANN ON TOP LOL",
                },
            }
        }
    
        PerformHttpRequest(notallowed, function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
    elseif(allowed ~= true) then
        local embed = {
            {
                ["color"] = 65322,
                ["title"] = "**ALLOWED TOKEN USED**",
                ["description"] = "`"..servername.."` Using Allowed token("..Config.License..") ON RESOURCE **"..resourcename.."** \n IP: `"..ip.."`",
                ["footer"] = {
                    ["text"] = "YEHONATANN ON TOP LOL",
                },
            }
        }
        print("^2Thanks for purchasing from us | Razer's Shop | Vaild Token")
        PerformHttpRequest(allowedurl, function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
    end
  end
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if(Config.License == nil) then
        PerformHttpRequest("https://api.myip.com/", function(err, text, headers)
            local data = json.decode(text)
            local ip = data.ip
            sendToDiscord(true , GetCurrentResourceName() , ip)
            print("^1 Using Invailed license nice try mate | Razer")
            Citizen.Wait(1000)
            os.exit()
    end, 'GET', '')      
    elseif(Config.License ~= nil) then
        PerformHttpRequest("https://api.myip.com/", function(err, text, headers)
            local data = json.decode(text)
            local ip = data.ip
            PerformHttpRequest("https://yehonatann.xyz/security/check_license.php?token="..Config.License.."&serverip="..ip, function(err, gay, headers)
               if(gay ~= 'yes') then
                    sendToDiscord(true , GetCurrentResourceName() , ip)
                    print("^1 Using Invailed license nice try mate | Razer")
                    Citizen.Wait(1000)
                ...
                sendToDiscord(false , GetCurrentResourceName() , ip)
               end
            end, 'GET', '') 
        end, 'GET', '')       
    end
