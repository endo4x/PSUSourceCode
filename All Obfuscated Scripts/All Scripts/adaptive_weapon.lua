--[[
	id: sowIF8DoXSrqCq4AOfGiE
	name: adaptive_weapon2
	description: adaptive_weapon2
	time1: 2021-05-04 05:37:25.895095+00
	time2: 2021-05-04 05:37:25.895096+00
	uploader: 1928
...
local success, csgo_weapons = pcall(require, 'gamesense/csgo_weapons')
if not success then error("Subscribe https://gamesense.pub/forums/viewtopic.php?id=18807", 2) end
...
local TK = ui.new_checkbox("RAGE", "Other", "Teammates-Fire")
local TK_Hotkey = ui.new_hotkey("RAGE", "Other", "Teammates-Fire Hotkey")
local options = ui.new_multiselect("RAGE", "Other", "[Lua]Prefer Baim", "Backwards/Forwards", "Moving targets", "Slow targets", "Shooting", "x2 HP", "<x HP", "Big desync", "Walking jitter desync", "Always on", "Override /X M")
local headaim_options = ui.new_multiselect("RAGE", "Other" , "[Lua]Prefer Headshot", "Sideways targets", "Crouching targets", "Fast targets", "Shooting", "Small desync", "Override /X M")
local force_options = ui.new_multiselect("RAGE", "Other" , "[Lua]Force Baim", "Backwards/Forwards", "Sideways targets", "Slow targets", "Shooting", "x1 HP", "x2 HP", "<x HP", "Walking jitter desync", "1 miss", "2 miss", "On Double Tap", "On Hide Shot", "Override /X M")
local safe_options = ui.new_multiselect("RAGE", "Other" , "[Lua]Force Safety", "Backwards/Forwards", "Sideways targets", "Slow targets", "Shooting", "x1 HP", "x2 HP", "<x HP", "Walking jitter desync", "1 miss", "2 miss", "On Double Tap", "On Hide Shot", "Override /X M")
...
local weapon_targets = {"Auto", "AWP", "Scout", "Pistol", "Rifle", "SMG", "R8 Revolver", "Deagle", "Heavy", "Other", "Taser Zeus"}
local weapon_active = ui.new_checkbox("Rage", "Aimbot", "Enabled Adaptive Weapons")
local global_c = ui.new_color_picker("Rage", "Aimbot", "Global Color C_S", 0, 191, 255, 255)
local dmg_color_label = ui.new_label("Rage", "Aimbot", "Damage Indicators Color")
local dmg_c = ui.new_color_picker("Rage", "Aimbot", "DMG Color", 0, 255,27,255)
...
local head_shoter_c = ui.new_color_picker("Rage", "Other", "Head Color c", 255, 0, 0, 255)
...
for i=1, #weapon_targets do
    active[i] = {
        ...
        ["unsafe_hitbox"] = ui.new_multiselect("Rage", "Aimbot", "[" .. weapon_targets[i] .. "]Avoid UnSafety Hitboxes", {"Head", "Chest", "Stomach", "Arms", "Legs"}),
        ["unsafe_hitbox_ove"] = ui.new_multiselect("Rage", "Aimbot", "[" .. weapon_targets[i] .. "]Override Avoid UnSafety Hitboxes", {"Head", "Chest", "Stomach", "Arms", "Legs"}),
        ...
        ["lua_prefer_baim"] = ui.new_multiselect("Rage", "Other", "[" .. weapon_targets[i] .. "]Prefer Baim", {"Backwards/Forwards", "Moving targets", "Slow targets", "Shooting", "x2 HP", "<x HP", "Big desync", "Walking jitter desync", "Always on", "Override /X M"}),
        ["lua_prefer_head"] = ui.new_multiselect("Rage", "Other" , "[" .. weapon_targets[i] .. "]Prefer Head Shot", {"Sideways targets", "Crouching targets", "Fast targets", "Shooting", "Small desync", "Override /X M"}),
        ["lua_force_baim"] = ui.new_multiselect("Rage", "Other" , "[" .. weapon_targets[i] .. "]Force Baim", {"Backwards/Forwards","Sideways targets", "Slow targets", "Shooting", "x1 HP", "x2 HP", "<x HP", "Walking jitter desync", "1 miss", "2 miss", "On Double Tap", "On Hide Shot", "Override /X M"}),
        ["lua_force_safe"] = ui.new_multiselect("Rage", "Other" , "[" .. weapon_targets[i] .. "]Force Safe Point", {"Backwards/Forwards","Sideways targets", "Slow targets", "Shooting", "x1 HP", "x2 HP", "<x HP", "Walking jitter desync", "1 miss", "2 miss", "On Double Tap", "On Hide Shot", "Override /X M"}),
        ...
        ["misc_label"] = ui.new_label("Rage", "Other", "------------ Miscellaneous Weapons Functions ------------"),
        ...
        ["lua_condition"] = ui.new_multiselect("Rage", "Other" , "[" .. weapon_targets[i] .. "]Weapon Condition", {"Visible Damage", "Override Damage /X1", "Override Damage /X2", "Override Damage /X3", "Override Hitbox", "Visible Hitchance", "Override Hitchance", "NoScope Hitchance", "Override Multi-Point", "Override UnSafety Hitbox", "Prefer Hitbox Attack Condition", "Customized Double Tap"}),
...
local resert_attack_key = ui.new_hotkey("Rage", "Other" ,"Reset Prefer Attack Hitbox Keys")
...
client.set_event_callback("paint",function()
	...
	ui.set_visible(dmg_color_label, ui.get(weapon_active))
	ui.set_visible(dmg_c, ui.get(weapon_active))
	ui.set_visible(lua_resolver, ui.get(weapon_active))
	ui.set_visible(active_on_switch, ui.get(weapon_active))
	ui.set_visible(resert_attack_key, ui.get(weapon_active))
	ui.set_visible(weapon_indicator, ui.get(weapon_active))
	ui.set_visible(auto_peeking_key, ui.get(weapon_active))
	...
	ui.set_visible(lua_headbaim_indicator, ui.get(weapon_active))
	ui.set_visible(override_damage_key_1, ui.get(weapon_active))
	ui.set_visible(override_damage_key_2, ui.get(weapon_active))
	ui.set_visible(override_damage_key_3, ui.get(weapon_active))
	ui.set_visible(override_multipoint_key, ui.get(weapon_active))
	ui.set_visible(disabled_de_on_greaner, ui.get(weapon_active))
	ui.set_visible(disabled_scope_hitchance, ui.get(weapon_active))
	ui.set_visible(override_unsafehitbox_key, ui.get(weapon_active))
	ui.set_visible(enabled_adaptive_hitchance, ui.get(weapon_active))
	ui.set_visible(head_shoter_c, ui.get(weapon_active) and ui.get(indicator))
	ui.set_visible(safe_shoter_label, ui.get(weapon_active) and ui.get(indicator))
	ui.set_visible(head_shoter_label, ui.get(weapon_active) and ui.get(indicator))
	ui.set_visible(forcesafe_shoter_c, ui.get(weapon_active) and ui.get(indicator))
	ui.set_visible(prefer_shoter_label, ui.get(weapon_active) and ui.get(indicator))
	ui.set_visible(forcebaim_shoter_c, ui.get(weapon_active) and ui.get(indicator))
	ui.set_visible(lua_resolver_logs, ui.get(weapon_active) and ui.get(lua_resolver))
	ui.set_visible(preferbaim_shoter_c, ui.get(weapon_active) and ui.get(indicator))
	ui.set_visible(lua_resolver_types, ui.get(weapon_active) and ui.get(lua_resolver))
	ui.set_visible(preferbaim_shoter_label, ui.get(weapon_active) and ui.get(indicator))
	...
	ui.set_visible(keybinds_listbox, ui.get(weapon_active) and has_value(ui.get(weapon_indicator), "Active Boxer"))
...
local function weapons_name(c)
	...
	local headaim_check = has_value(ui.get(weapon_indicator), "Prefer HeadBaim Boxer") and has_value(ui.get(active[weapons_active_idx]["lua_condition"]), "Prefer Hitbox Attack Condition") and entity.is_alive(local_player) and ui.get(weapon_active)
	if headaim_check then
		...
		renderer.text(x_4 + 10, y_4 + 2, 0, 191, 255, 255, "", 0, "HEADBAIM")
		renderer.text(#enemy_players <= 0 and x_4 + 80 or x_4 + 85, y_4 + 2, #enemy_players <= 0 and weapon_r or 255, #enemy_players <= 0 and weapon_g or 0, #enemy_players <= 0 and weapon_b or 0, alpha, "", 0, #enemy_players <= 0 and "WAITING..." or "ACTIVE")
	end
	...
	for i = 1, #enemy_players do
		...
		if has_value(ui.get(weapon_indicator), "Distance") and entity.is_alive(enemy_player) and enemy_player and entity.is_alive(local_player) and ui.get(weapon_active) then
		...
		if has_value(ui.get(weapon_indicator), "Resolver Angles") and entity.is_alive(enemy_player) and enemy_player and entity.is_alive(local_player) and ui.get(weapon_active) then
		...
		if enemy_player >= 1 and headaim_check then
			renderer.text(x_4 + 10, y_4 + 5 + (i * 10), G_Color[1], G_Color[2], G_Color[3], 180, "", 0,"->")
			renderer.text(x_4 + 25, y_4 + 5 + (i * 10), G_Color[1], G_Color[2], G_Color[3], 180, "", 0, "Target: ")
			renderer.text(x_4 + 65, y_4 + 5 + (i * 10), G_Color[1], G_Color[2], G_Color[3], 210, "", 0, entity.get_player_name(enemy_player))
			renderer.text(x_4 + 148, y_4 + 5 + (i * 10), 0, 255, 0, 255, "", 0, "● ")
			renderer.text(x_4 + 156, y_4 + 5 + (i * 10), G_Color[1], G_Color[2], G_Color[3], 180, "", 0, player_resolver_flags)
		end
	end
	...
	for i=1, #weapon_targets do
		...
		ui.set_visible(active[i]["faster_peeking"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		...
		ui.set_visible(active[i]["hitbox_override"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Override Hitbox"))
		...
		ui.set_visible(active[i]["damage_visible"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Visible Damage"))
		ui.set_visible(active[i]["damage_override_1"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Override Damage /X1"))
		ui.set_visible(active[i]["damage_override_2"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Override Damage /X2"))
		ui.set_visible(active[i]["damage_override_3"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Override Damage /X3"))
		ui.set_visible(active[i]["hitchance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		ui.set_visible(active[i]["vis_hitchance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Visible Hitchance"))
		ui.set_visible(active[i]["override_hitchance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Override Hitchance"))
		ui.set_visible(active[i]["enabled_smart_footaim"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		ui.set_visible(active[i]["adaptive_damage_override"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		ui.set_visible(active[i]["unsafe_hitbox"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		ui.set_visible(active[i]["unsafe_hitbox_ove"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Override UnSafety Hitbox"))
		ui.set_visible(active[i]["noscope_hitchance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(disabled_scope_hitchance), ui.get(weapons_acitve_visible)) and has_value(ui.get(active[i]["lua_condition"]), "NoScope Hitchance"))
		...
		ui.set_visible(active[i]["dynamic_types"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["dynamic_mp"]))
		ui.set_visible(active[i]["mp_scale_ove"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Override Multi-Point"))
		...
		ui.set_visible(active[i]["dt_hitchance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["dt_enabled"]) and not ui.get(enabled_adaptive_hitchance) and has_value(ui.get(active[i]["lua_condition"]), "Customized Double Tap"))
		...
		ui.set_visible(active[i]["enabled_dis_status"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		ui.set_visible(active[i]["target_select_dis"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["enabled_dis_status"]))
		ui.set_visible(active[i]["target_hitbox_distance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["enabled_dis_status"]))
		ui.set_visible(active[i]["mp_hitbox_distance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["enabled_dis_status"]))
		ui.set_visible(active[i]["damage_distance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["enabled_dis_status"]))
		ui.set_visible(active[i]["hitchance_distance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["enabled_dis_status"]))
		ui.set_visible(active[i]["mp_modes_dis"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["enabled_dis_status"]))
		ui.set_visible(active[i]["mp_scale_dis"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["enabled_dis_status"]))
		ui.set_visible(active[i]["enemy_distance"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["enabled_dis_status"]))
		ui.set_visible(active[i]["lua_prefer_baim"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_prefer_head"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_force_baim"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_force_safe"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_prediction_baim"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_auto_stop"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		ui.set_visible(active[i]["lua_desync_limit"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and get_desync_select and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_range"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and get_sideways_select and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_jitter_sensitivity"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and get_jitter_select and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_min_hp"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and get_health_select and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_auto_stop_speed"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["lua_auto_stop"]))
		ui.set_visible(active[i]["lua_condition"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		ui.set_visible(active[i]["lua_preferbaim_m"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_prefer_baim"]), "Override /X M") and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_preferhead_m"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_prefer_head"]), "Override /X M") and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_forcebaim_m"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_force_baim"]), "Override /X M") and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_forcesafety_m"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and has_value(ui.get(active[i]["lua_force_safe"]), "Override /X M") and has_value(ui.get(active[i]["lua_condition"]), "Prefer Hitbox Attack Condition"))
		ui.set_visible(active[i]["lua_auto_peek"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i])
		ui.set_visible(active[i]["lua_auto_peek_triggers"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["lua_auto_peek"]))
		ui.set_visible(active[i]["lua_auto_peek_shots"], ui.get(weapon_active) and ui.get(weapons_acitve_visible) == weapon_targets[i] and ui.get(active[i]["lua_auto_peek"]) and has_value(ui.get(active[i]["lua_auto_peek_triggers"]), "X Shots"))
	...
	local multi_visible_damage = #enemy_players == 0 and false or has_value(ui.get(active[weapons_active_idx]["lua_condition"]), "Visible Damage") and weapon_visible == true
	...
	local multi_visible_hitchance = has_value(ui.get(active[weapons_active_idx]["lua_condition"]), "Visible Hitchance") and weapon_visible == true
	...
	local status = ui.get(manual_state)
	if status == 1 or automatic_override_1 then
		override_damage_types = ui.get(active[weapons_active_idx]["damage_override_1"])
		override_damage_flags = "X1 Ove"
		ui.set(min_dmg, override_damage_types)
	elseif status == 2 or automatic_override_2 then
	...
	if status == 0 then ui.set(min_dmg, lua_damage) end
	...
	ui.set(active_config_label, "Active Weapon Config Name: ".. active_weapons)
	...
	--[[
	if not ui.get(weapon_active) or not ui.get(lua_resolver) then
	...
	if has_value(ui.get(weapon_indicator), "Active Weapons") then
		client.draw_indicator(ctx, weapon_r, weapon_g, weapon_b, weapon_a, "Weapon: ".. active_weapons)
	end
	...
	if has_value(ui.get(weapon_indicator), "Active Boxer") then
		...
		renderer.rectangle(real_x, real_y, 145, 25, 20, 20, 20, 150)
		renderer.rectangle(real_x, real_y, 145, 2, weapon_r, weapon_g, weapon_b, weapon_a)
		renderer.text(real_x + 75, real_y + 13, 255, 255, 255, 255, "c", 0, "Active Weapon")
		weapon_icon:draw(real_x + 25, real_y + 28, nil, 22)
		steam_avatar:draw(real_x, real_y + 30, nil, 18)
		renderer.text(real_x + 100, real_y + 25, 255, 255, 255, 255, "", 0, "HTC: " .. ui.get(min_htc))
		renderer.text(real_x + 100, real_y + 38, 255, 255, 255, 255, "", 0, "DMG: " .. ui.get(min_dmg))
	...
	if has_value(ui.get(weapon_indicator), "Min Damage") then
		if status > 0 then damage_status = "X1 Ove" end
		if damage_status == "Def" then
			client.draw_indicator(ctx, DMG_Color[1], DMG_Color[2], DMG_Color[3], 255, "Min DMG: " .. ui.get(min_dmg))
		elseif damage_status == "Vis" then
			client.draw_indicator(ctx, DMG_Color[1], DMG_Color[2], DMG_Color[3], 255, "Min DMG: " .. ui.get(min_dmg))
		elseif damage_status == "X1 Ove" or damage_status == "X2 Ove" or damage_status == "X3 Ove" then
			client.draw_indicator(ctx, 255, 0, 0, 255, "Min DMG: " .. ui.get(min_dmg))
		elseif damage_status == "Dis" or damage_status == "Air" then
			client.draw_indicator(ctx, DMG_Color[1], DMG_Color[2], DMG_Color[3], 255, "Min DMG: " .. ui.get(min_dmg))
		end
	end

	if ui.get(ref.hideshot) and ui.get(ref.os[1]) and ui.get(ref.os[2]) then
		renderer.indicator(0, 191, 255, 255, "Hide-shot")
	end
end
...
client.set_event_callback("aim_miss", function(e)
	client.update_player_list()
	local targets_players = entity.get_players(true)
	if not ui.get(weapon_active) or not ui.get(lua_resolver) or not entity.is_alive(entity.get_local_player()) or #targets_players == 0 then
		for i = 1, #targets_players do
			local enemy_targets = targets_players[i]
			plist.set(enemy_targets, "Force body yaw", false)
			plist.set(enemy_targets, "Force body yaw value", 0)
...
client.set_event_callback("round_end", function(e)
	if not ui.get(weapon_active) or not ui.get(lua_resolver) or not entity.is_alive(entity.get_local_player()) then
		return
	end
...
client.set_event_callback("paint", function(e)
	if not ui.get(weapon_active) then
		return
	end
	...
	local auto_peeking_is_enabled = ui.get(active[weapons_active_idx]["lua_auto_peek"]) and ui.get(auto_peeking_key) and peek_pos_x ~= nil and entity.is_alive(entity.get_local_player())
	...
	if wx ~= nil then
		local weapon_r, weapon_g, weapon_b, weapon_a = ui.get(global_c)
		draw_circle_3d(peek_pos_x, peek_pos_y, peek_pos_z, 14, weapon_r, weapon_g, weapon_b, weapon_a, 3, 2, false, 0, 1, weapon_r, weapon_g, weapon_b, weapon_a * 0.5)
	end
end)

local function auto_peek_run_command(cmd)
	if not ui.get(weapon_active) or not ui.get(active[weapons_active_idx]["lua_auto_peek"]) then
		return
	end

	if ui.get(auto_peeking_key) then
		local local_player = entity.get_local_player()
	...
	hotkey_prev = ui.get(auto_peeking_key)
end
...
local function hitbox_fast_indicator(c)
	...
	if not ui.get(weapon_active) or not ui.get(indicator) or not entity.is_alive(entindex) then
		return
	end
...
local function run_command(cmd)
	...
	if ui.get(TK) and ui.get(TK_Hotkey) then
	    cvar.mp_teammates_are_enemies:set_int(1)
	else
	    cvar.mp_teammates_are_enemies:set_int(0)
	end

	if local_player == nil or not has_value(ui.get(active[weapons_active_idx]["lua_condition"]), "Prefer Hitbox Attack Condition") then
		return
	end
...
client.set_event_callback("shutdown",function() cvar.mp_teammates_are_enemies:set_int(0) end)