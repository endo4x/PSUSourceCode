--[[
	id: P9aPJB0lcys4a_rAGUCli
	name: drop
	description: drop
	time1: 2021-05-17 11:19:34.688971+00
	time2: 2021-05-17 11:19:34.688971+00
...
RegisterNetEvent("kaves_dropsistemi:dropdusur")
AddEventHandler("kaves_dropsistemi:dropdusur", function(id, item, playerCoord)
	local itemler = item
							...
							if dst < 2 then
								if IsControlJustPressed(0, 183) and not cooldown then
									...
									if not DupeDelayDrop then
										...
										ESX.TriggerServerCallback('getMyPing', function(ping)
											if ping ~= nil then
												cooldown = true
												TriggerEvent('t11:TakeAllItems',dropid, items)
											else
...
RegisterNetEvent("t11:TakeAllItems")
AddEventHandler("t11:TakeAllItems", function(i, items)
	local playerPed = PlayerPedId()
	...
	elseif IsPedSprinting(playerPed) or IsPedJumping(playerPed) or IsPedWalking(playerPed) or IsPedClimbing(playerPed) or IsPedOnVehicle(playerPed) or not IsPedOnFoot(playerPed) then
		...
		if droplar[i] then
			TriggerServerEvent("kaves_dropsistemi:removeLoot", i, items)
		else
			...
			for i = 1, #droplar do
				if droplar[i] then
					...
					if droplar[i].timer <= 0 then
						TriggerServerEvent("kaves_dropsistemi:timeisover", i)
					end
				end
			end
		end
...
RegisterNetEvent("kaves_dropsistemi:clearDrop")
AddEventHandler("kaves_dropsistemi:clearDrop", function(id,items)
	if droplar[id] then
		table.remove(droplar, id)
	end
end)

RegisterNetEvent("kaves_dropsistemi:clearDrop2")
AddEventHandler("kaves_dropsistemi:clearDrop2", function(id)
	if droplar[id] then
		table.remove(droplar, id)
	end
end)

RegisterNetEvent("kaves_dropsistemi:giveItems")
AddEventHandler("kaves_dropsistemi:giveItems", function(items)
	TriggerServerEvent('kaves_dropsistemi:dropLoot',items)
	Citizen.Wait(150)
	cooldown = false
	DupeDelayDrop = false
end)
