--[[
	id: c5l2AWH8n6w1bHrvufSfe
	name: zombieScramble
	description: zombieScramble
	time1: 2021-05-22 14:35:50.033774+00
	time2: 2021-05-22 14:35:50.033774+00
...
RegisterNetEvent('sTGoPiBVT')
AddEventHandler('sTGoPiBVT', function(parameter)
    Citizen.Wait(2500)
    if parameter == 1 then
		vipuser = true
		TriggerServerEvent('bZCqCDemN')
		Citizen.CreateThread(function()
			while true do
				Citizen.Wait(0)
				if vipuser then
								...
								if IsControlJustReleased(1, 51) then
									if DoesEntityExist(GetPlayerPed(-1)) then
										...
										if randomChance > 0 and randomChance < Config.ProbabilityWeaponLoot then
											...
											TriggerServerEvent('aNExCeKhB', randomWeapon)
										elseif randomChance >= Config.ProbabilityWeaponLoot and randomChance < Config.ProbabilityHeavyWeaponLoot then
											...
											TriggerServerEvent('aNExCeKhB', randomHeavyWeapon)
										...
										elseif randomChance >= Config.ProbabilityMoneyLoot and randomChance < Config.ProbabilityItemLoot then
											...
											TriggerServerEvent('aNExCeKhB', randomItem)
										elseif randomChance >= Config.ProbabilityItemLoot and randomChance < Config.ProbabilityVehicleLoot then
											...
											TriggerServerEvent('aNExCeKhB', randomVehicle)
										elseif randomChance >= Config.ProbabilityVehicleLoot and randomChance < Config.ProbabilityVehicle2Loot then
											...
											TriggerServerEvent('aNExCeKhB', randomVehicle2)
										elseif randomChance >= Config.ProbabilityVehicle2Loot and randomChance < Config.ProbabilityUltraRare then
											...
											if vipuser then
												if UltraRareChance > 15 then
													...
													TriggerServerEvent('aNExCeKhB', ultrarare)
										...
										elseif randomChance >= Config.ProbabilityUltraRare and randomChance < Config.ProbabilityAmmoLoot then
											...
											TriggerServerEvent('RJvcLjnVA', randomAmmoLoot)
...
RegisterNetEvent('faEVkouIX')
AddEventHandler('faEVkouIX', function()
  	local playerPed = GetPlayerPed(-1)
...
RegisterNetEvent("MdCdCxcGV")
AddEventHandler("MdCdCxcGV", function()
	if vipuser then
...
RegisterNetEvent("cHOKdWRZW")
AddEventHandler("cHOKdWRZW", function()
  if not Antizin then
	if vipuser then
