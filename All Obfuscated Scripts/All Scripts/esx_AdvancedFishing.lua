--[[
	id: 8eq32XuF42EoThdxRLudc
	name: esx_AdvancedFishing
	description: script pesca client
	time1: 2021-04-24 13:26:02.87195+00
	time2: 2021-04-24 13:26:02.871951+00
...
-- 		AddTextComponentString("Noleggio barche")
	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				fishing = false
				ESX.ShowNotification("~r~Ho smesso di pescare")
			end
			if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~Il pesce si è liberato")
				end
			end
		end
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("~g~Il pesce sta abboccando \n ~h~Premi " .. correct .. " per prenderlo")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~y~Non puoi pescare da un veicolo")
	else
		if pos.y >= 5500 or pos.y <= -3000 or pos.x <= -2500 or pos.x >= 3000 then
			ESX.ShowNotification("~g~Fishing started")
			TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_STAND_FISHING", 0, true)
			fishing = true
		else
			ESX.ShowNotification("~y~Devi allontanarti dalla riva")
		end
	end
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('Premi E per affittare una barca')
...
function OpenBoatsMenu(x, y , z)
	...
	if PlayerData.job.name == "police" then
		table.insert(elements, {label = '<span style="color:green;">Barca Carabinieri</span>', value = 'police'})
	end
	...
	if data.current.value == 'boat' then
		...
		TriggerEvent("chatMessage", 'Hai noleggiato una barca per', {255,0,255}, '$' .. 2500)
	...
	if data.current.value == 'boat2' then
		...
		TriggerEvent("chatMessage", 'Hai noleggiato una barca per', {255,0,255}, '$' .. 5500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat3' then
		...
		TriggerEvent("chatMessage", 'Hai noleggiato una barca per', {255,0,255}, '$' .. 6000)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerEvent("chatMessage", 'Hai noleggiato una barca per', {255,0,255}, '$' .. 7500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat5' then
		...
		TriggerEvent("chatMessage", 'Hai noleggiato una barca per', {255,0,255}, '$' .. 4500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat6' then
		...
		TriggerEvent("chatMessage", 'Hai noleggiato una barca per', {255,0,255}, '$' .. 3500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'police' then
		...
		TriggerEvent("chatMessage", 'Hai preso una barca')
		SetPedCoordsKeepVehicle(ped, x, y , z)
