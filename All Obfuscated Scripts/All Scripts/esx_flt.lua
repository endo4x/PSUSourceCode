--[[
	id: 8pAlaowvMCUDiEuRI-l46
	name: esx_flt
	description: lavoro muletto
	time1: 2021-04-24 17:39:11.336917+00
	time2: 2021-04-24 17:39:11.336917+00
...
Citizen.CreateThread(function()
    ...
    if Player.Data.job.name == 'import' then
...
function StartWork()
    Player.IMPORT.Plate = GetVehicleNumberPlateText(GetVehiclePedIsIn(Player.Ped, false))
    Player.Working = true
...
function SpawnPallet()
    ...
    if not Player.Pallet then
        ...
        ESX.Game.SpawnObject(Config.Pallet, Zones.Pickup.Pos, function(pallet)
            ...
            Utils.DrawZoneBlip("Pickup", 1, "Punto di raccolta pallet")
...
function PickupPallet()
    ...
    Utils.DrawZoneBlip("Drop", 1, "Punto di consegna pallet")
...
function WreckPallet()
    ...
    -- Stop IMPORT
    SetVehicleHandbrake(Player.IMPORT.Entity, true)

    -- Play animation
    Utils.PlayAnimation("anim@mp_player_intcelebrationmale@face_palm", "face_palm")

    -- Allow IMPORT to move again
    Citizen.SetTimeout(3000, function()
        SetVehicleHandbrake(Player.IMPORT.Entity, false)
    end)    
...
function SpawnIMPORT()
    ESX.Game.SpawnVehicle(Config.Vehicle, Config.Zones.IMPORT.Pos, Config.Zones.IMPORT.Heading, function(import)
        SetVehicleNumberPlateText(import, math.random(1000, 9999))
        Wait(100)
        ...
        TaskWarpPedIntoVehicle(Player.Ped, import, -1)
    end)
end

-- Return your IMPORT
function StoreIMPORT(force)
    if Player.IMPORT then
        Player.Working = false

        -- Player returned IMPORT to warehouse
        if force or (IsPedInAnyVehicle(Player.Ped) and Player.IMPORT.Plate == GetVehicleNumberPlateText(GetVehiclePedIsIn(Player.Ped, false))) then
            TriggerServerEvent('esx_flt:getPaid', Player.Delivered * Config.Pay)

            ESX.Game.DeleteVehicle(Player.IMPORT.Entity)
            Player.IMPORT = false
            ...
            ESX.ShowNotification('~r~Devi restituire il tuo muletto per essere pagato!!')
        end
    end
end
...
function UpdateBlips()
    ...
    if Zones.Pickup.Active then
        Utils.DrawZoneBlip("Pickup", 1, "Punto di prelievo pallet")
    end   
        ...
        while true do
            if Ready and Player.Authorized then
                if Player.Working and Zones.Drop.Ready then
                    if Player.Pallet then
                        Player.AtPallet = IsEntityAtEntity(Player.IMPORT.Entity, Player.Pallet.Entity, 3.0, 3.0, 3.0, 0, 1, 0)
                    end
...
function StartInteractionThread()
    Citizen.CreateThread(function()
        while true do
            ...
            if Ready and Player.Authorized then
                if Player.IMPORT then
                    if not IsPedSittingInVehicle(Player.Ped, Player.IMPORT.Entity) then
                        Player.IMPORT.Active = false

                        -- IMPORT blip
                        Utils.AddBlip(Player.IMPORT, 255, _U('import'))

                        RemoveBlip(Zones.Return.Blip)
                    else
                        if Player.IMPORT.Blip then
                            UpdateBlips()
                            RemoveBlip(Player.IMPORT.Blip)
                        end
                        Player.IMPORT.Active = true

                        -- Start work when player enters IMPORT
    ...
    Citizen.CreateThread(function()
        while true do
            if Ready and Player.Authorized then
                ...
                if Player.OnDuty then
                    if not Player.IMPORT then
                        if Distance.Garage <= Config.DrawDistance then
                            -- IMPORT Garage Marker
    ...
    Citizen.CreateThread(function()
        while true do
            if Ready and Player.Authorized then
                ...
                if Player.OnDuty then
                    if Distance.Garage <= Zones.Garage.Size.x and not Player.IMPORT then
                        Hint.Zone = 'Garage'
                        Hint.Display = true
                        Hint.Message = _U('enter_import')
                    end
                        
                    if Distance.Return <= Zones.Garage.Size.x and Player.IMPORT and Player.IMPORT.Active then
                        Hint.Zone = 'Return'
                        Hint.Display = true
                        Hint.Message = _U('return_import')
                    end  
