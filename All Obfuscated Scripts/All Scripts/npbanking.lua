--[[
	id: bP5U9ZWEtTttN-xY6ld0c
	name: np-banking
	description: client
	time1: 2021-04-27 00:52:15.813182+00
	time2: 2021-04-27 00:52:15.813182+00
...
AddEventHandler('bank:checkATM', function()
  if IsNearATM() then
    ...
    TriggerEvent("DoLongHudText","Няма банкомат наблизо.",2)
  end
end)
...
-- Banks
local banks = {
  {name="Банка", id=108, x=150.266, y=-1040.203, z=29.374},
  ...
  {name="Банка", id=108, x=1176.0833740234, y=2706.3386230469, z=38.157722473145},
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["np-taskbar"]:taskBar(3000,"Взимате картата")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["np-taskbar"]:taskBar(1000,"Взимате картата")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
          ...
          while cdst < 1.5 do
            ...
            cdst = #(vector3(banks[scanid].x, banks[scanid].y, banks[scanid].z) -  vector3(plyCoords["x"], plyCoords["y"], plyCoords["z"]))
              ...
              if ClosedBanks[scanid] then
                DrawText3D(banks[scanid].x, banks[scanid].y, banks[scanid].z,"Банката е затворена.")
              else
                 DrawText3D(banks[scanid].x, banks[scanid].y, banks[scanid].z,"["..Controlkey["generalUse"][2].."] за Банката.")
