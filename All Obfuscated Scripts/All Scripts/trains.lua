--[[
	id: Ewm0JIdGW3JXrHRKM9B_4
	name: trains
	description: vlakove
	time1: 2021-04-29 00:38:19.760029+00
	time2: 2021-04-29 00:38:19.760029+00
...
AddEventHandler("AskForTrainConfirmed", function()
	...
	if metroDist < countryDist then
		if metroDist < 25.0 then 
			local finished = exports["np-taskbar"]:taskBar(60000,"Изчакване на влака",false,true)	
			if finished == 100 then
				StartMetro(closestMetro)
				TriggerEvent('chatMessage', 'Томас:', { 0, 0, 0 }, ' Choo Choo..')
			end
		else
			TriggerEvent("DoLongHudText","Не сте близо до гара!",2) 
		end
	else
...
AddEventHandler('Trains:ToggleTainsBlip', function()
   ...
   for _, item in pairs(trainstations) do
        if not showTrainStations then
			...
			AddTextComponentString("Метро гара")
    ...
    for _, item in pairs(trainstations2) do
        if not showTrainStations then
			...
			SetBlipAsShortRange(item.blip, true)
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString("Гара в провинцията")
			EndTextCommandSetBlipName(item.blip)
        end
    end
		...
		if(IsControlJustReleased(1,  38))then
			...
			if IsThisModelATrain( GetEntityModel(targetVehicle) ) and not intrain then
				...
				if IsPedInAnyTrain(playerped) then
					intrain = true
					TriggerEvent("DoLongHudText","Качихте се успешно в влака/трамвая!",1)
				else
					TriggerEvent("DoLongHudText","Съжеляваме, влака е пълен!",2)
				end
				Citizen.Wait(1500)
				justborded = false
			else
...
function StartMetro(coords)
	...
	local models = { 1030400667, 184361638, 642617954, 920453016, 240201337,
			3517691494, 586013744, 868868440 }
	...
	TriggerEvent('chatMessage', 'THOMAS:', { 0, 0, 0 }, ' Влака тръгва след 30 секунди.')
