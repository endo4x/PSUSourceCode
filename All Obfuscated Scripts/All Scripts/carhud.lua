--[[
	id: uucCKO3MhT1L_Hn6Nd0aV
	name: carhud
	description: car
	time1: 2021-04-27 01:02:26.040557+00
	time2: 2021-04-27 01:02:26.040558+00
...
AddEventHandler('car:doors', function(closeType,door)
	...
	if not isInVehicle then
	    ...
	    if DoesEntityExist(Driver) and not IsPedAPlayer(Driver) then
	        TriggerEvent("DoLongHudText","МПС-то е заключено!",2)
	        return
	    end
    ...
    repeat
        ...
        if distance < 3.0 and not IsPedAPlayer(ped) then
        	if hashDeathToText['' .. GetPedCauseOfDeath(ped) .. ''] ~= nil then
	        	TriggerEvent("DoLongHudText","Изглежда те са получили наранявания от  " .. hashDeathToText['' .. GetPedCauseOfDeath(ped) .. ''])
	        else
	        	TriggerEvent("DoLongHudText","Изглежда че човека е починал от неизвесност")
	        end
    ...
    repeat
        ...
        if distance < 2.0 and not IsPedAPlayer(ped) then
        	if hashDeathToText['' .. GetPedCauseOfDeath(ped) .. ''] ~= nil then
	        	...
	        	TriggerEvent("DoLongHudText","Изглежда, че са умрели от неизвестни причини")
	        end
        	ResurrectPed(ped)
            ReviveInjuredPed(ped)
            ClearPedTasksImmediately(ped)
...
function EnableCruise(get_ped_veh)
	...
	TriggerEvent("DoLongHudText",'Круиз Контрола е пуснат',11)
end
...
function DisableCruise(showMsg)
	...
	if showMsg then
		TriggerEvent("DoLongHudText",'Круиз Контрола е деактвириан',11)	
	end
end
...
--v_med_bed2=2117668672
	...
	if DoesEntityExist(objFound) then
		TriggerEvent("DoLongHudText","Натисни [E] за да станеш от леглото.",1)
...
AddEventHandler("client:newStress",function(positive,alteredValue)
	...
	if positive then
		TriggerEvent("DoShortHudText",'Стресиран/а сте',6)
	else
		TriggerEvent("DoShortHudText",'По-успокоен/а сте',6)
	end
