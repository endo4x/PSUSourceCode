--[[
	id: 8I_XRL43wfc3SULq1dT3C
	name: poop
	description: 4/25/21
	time1: 2021-04-26 03:28:01.36271+00
	time2: 2021-04-26 03:28:01.362711+00
...
local mshit = {
	...
	label18 = ui.new_label("AA", "Anti-aimbot angles", "Bodyyaw Indicator Color"),
	color18 = ui.new_color_picker("AA", "Anti-aimbot angles", 'Bodyyaw Indicator Color Bar', 255, 255, 255, 255),
...
local function antiAim()
	...
	--lby_yaw = math.floor(entity.get_prop(entity.get_local_player(), "m_flPoseParameter", 11) * 120 - 60)
	local move
	local duck
    ...
    if ui.get(mshit.option) == "Manual AA" then
		...
		if ui.get(mshit.idealoption) == "Jitter" then
			if check_double_tap and doubletap_charged or check_hide_shots or ui.get(ref.fake_lag) < 10 then
				...
				ui.set(jyawslide, -12)
				if ui.get(mshit.enable) then
					ui.set(bodyyaw2, math.random(-90, 90)) --ui.set(bodyyaw2, lby2)	
					ui.set(ref.freestanding_byaw, false)
				else
...
client.set_event_callback('run_command', function()
    ...
    for i=1,#enemies do
		...
		if not entity.is_alive(player) then 
			...
			if ui.get(holding_e) or (entity.get_classname(weapon) == "CKnife" and distance <= 300) then
				ui.set(mshit.idealoption, "EHold")
			elseif (bit.band(entity.get_prop(entity.get_local_player(), "m_fFlags"), 1) == 1) then
				if (duck == true) then
