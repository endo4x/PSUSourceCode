--[[
	id: z4xxRpR_zx4p_o3-_YbPa
	name: np-emotes cl_menu
	description: ca
	time1: 2021-04-27 01:07:41.229344+00
	time2: 2021-04-27 01:07:41.229345+00
...
WarMenu.CreateMenu("emotes", "Анимации")
WarMenu.SetSubTitle("emotes", "Анимации Лист")
...
WarMenu.CreateSubMenu("favorites", "emotes", "Любими (Натиснете F за да премахнете любима анимация)")
...
local function DrawMenu()
    if WarMenu.Button("Страница:", selected_page.."/"..#p_anims, {r = 135, g = 206, b = 250, a = 150}) then ClearPedTasks(PlayerPedId()) playing_emote = false end
    if WarMenu.Button("Любими", "Натисни F за да сложиш любим emote", {r = 135, g = 206, b = 250, a = 150}) then WarMenu.OpenMenu("favorites") end
    local key = ""
    for i,v in ipairs(currentKeys) do
        if v.key[2] ~= nil and "Спри Анимацията" == v.anim then
             key = "Shift+"..Keys[v.key[1]]
        elseif v.key[1] ~= nil and "Спри Анимацията" == v.anim then
           key = Keys[v.key[1]]
        end
    end 
    if WarMenu.Button("Спри Анимацията",key, {r = 135, g = 206, b = 250, a = 150}) then ClearPedTasks(PlayerPedId()) playing_emote = false end
...
local function DrawFavorites()
    if WarMenu.Button("Спри Анимацията", nil, {r = 135, g = 206, b = 250, a = 150}) then ClearPedTasks(PlayerPedId()) playing_emote = false end
    if WarMenu.MenuButton("Обратно", "emotes", {r = 135, g = 206, b = 250, a = 150}) then end
...
function addKey(anim,keys)
    if anim == "Страница:" or anim == "Любими" or anim == "Обратно" then return end
    for i,v in ipairs(currentKeys) do
        if keys[2] ~= nil then
...
function DrawText3Ds(x,y,z, text)
	...
	if onScreen then
        SetTextScale(0.15 * scale, 0.15 * scale)
        SetTextFont(4)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened("emotes") then
            ...
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.4,"~g~Enter~s~ Пускане на Анимацията")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.3,"~g~Arrows~s~ Смяна на страниците")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.2,"~g~Backspace~s~ Излизане")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.1,"~g~F2-F10~s~ Запазва Анимация")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"],"~g~Shift~s~ със F1-F11. (F8 Не запазва)")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]-0.1,"Можете и да напишете ~g~/e името на анимацията~s~ в чата")
