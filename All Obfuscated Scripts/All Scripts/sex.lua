--[[
	id: WZ-7l2zYjEyk4JZh5IDDu
	name: sex
	description: sex
	time1: 2021-06-29 08:49:43.474978+00
	time2: 2021-06-29 08:49:43.474979+00
...
local function set_anti_aim(pitch, yaw, yaw_add, yawj, yawj_add, bodyyaw, bodyyaw_add, fs, fakelimit)
...
local function anti_aim()
    if includes(menu.options, "Dodge on slowwalk") and ui_get(refs.slowwalk[1]) and ui_get(refs.slowwalk[2]) then
        set_anti_aim("Minimal", "180", 9, "Off", 0, "Static", 28, false, client.random_int(17, 20))
        aa_state = "dodge"
        safety = "safe"
    elseif vars.best_angle == 0 then
        if includes(menu.options, "Force static body yaw") then
            set_anti_aim("Minimal", "180", 8, "Off", 0, "Static", 90, true, 35)
            aa_state = "static"
            safety = "unsafe"
        else
            set_anti_aim("Minimal", "180", 2, "Off", 0, "Jitter", 20, false, 57)
            aa_state = "jitter"
            safety = "unsafe"
        end
    elseif vars.best_angle == 1 then
        if vars.misses[vars.best_enemy] == nil or vars.misses[vars.best_enemy] == 2 then
            ...
            set_anti_aim("Minimal", "180", 9, "Off", 0, "Static", 27, false, 27)
            aa_state = "ideal"
            safety = "safe"
        end
    else
        if vars.misses[vars.best_enemy] == nil then
            if includes(menu.options, "Low delta") then
                set_anti_aim("Minimal", "180", 9, "Off", 0, "Static", 90, false, 35)
                aa_state = "bruteforce"
                safety = "safe"
            else
                set_anti_aim("Minimal", "180", 1, "Off", 0, "Static", -90, false, 35)
                aa_state = "bruteforce"
                safety = "safe"
            end
        else
...
local function ideal_tick_ind()
    ...
    if antiaim_funcs.get_double_tap() then
        renderer_text(w / 2, h / 2 - 23, wn_clr.r, wn_clr.g, wn_clr.b, 255, "b", 0, "IDEAL TICK : CHARGED")
    else
        renderer_text(
            w / 2,
            h / 2 - 23,
            wn_clr.r, wn_clr.g, wn_clr.b,
