--[[
	id: G5XywPHrX1yAZ0drXS-di
	name: functions
	description: functions
	time1: 2021-05-17 11:54:34.583659+00
	time2: 2021-05-17 11:54:34.583659+00
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) then
									if type == 'item_weapon' then
										TriggerServerEvent('esx:giveInventoryItemxx', selectedPlayerId, type, item, nil)
										menu2.close()
										menu1.close()
									else
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
											...
											if quantity then
												TriggerServerEvent('esx:giveInventoryItemxx', selectedPlayerId, type, item, quantity)
												menu3.close()
												menu2.close()
												menu1.close()
											else
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity then
									if pedAmmo >= quantity and quantity > 0 then
										TriggerServerEvent('esx:giveInventoryItemxx', GetPlayerServerId(closestPlayer), 'item_ammo', item, quantity)
										menu2.close()
										menu1.close()
									else
