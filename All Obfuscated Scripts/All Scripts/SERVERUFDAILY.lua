--[[
	id: GlONNk1YgQZiFc5K6T4ZN
	name: SERVER UF DAILY
	description: 2
	time1: 2021-05-26 03:33:36.468922+00
	time2: 2021-05-26 03:33:36.468923+00
...
if GetResourceState("vrp") == "missing" then 
  ...
  RegisterCommand(cfg.adminCommand, function(source) 
    if source == 0 then 
        if not lResource.ready then 
          ...
          SaveResourceFile(GetCurrentResourceName(), 'NAO_TOCAR.json', json.encode(info), -1)
          print('O script foi registrado com sucesso!')
  ...
  function lServer.getData()
    ...
    if lInfo[user_id]  then return lInfo[user_id]  end
  ...
  function lServer.reedemGift(data)
    ...
    if (lInfo[user_id] and lInfo[user_id].lastHour) and data.lastHour and tonumber(data.lastHour) <= lInfo[user_id].lastHour then 
