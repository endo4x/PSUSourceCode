--[[
	id: HNnd3fkgNhx1GynZdqVwR
	name: aaaaaa
	description: ffsad
	time1: 2021-05-06 11:47:27.353265+00
	time2: 2021-05-06 11:47:27.353266+00
...
local function manual_aa_paint()
    ...
    if m_state == 1 then renderer.text(x/2 - 45, y / 2 - 2, r, g, b, 255, "+c", 0, "<") end
    if m_state == 1 then renderer.text(x/2 + 45, y / 2 - 2, 255, 255, 255, 255, "+c", 0, ">") end
    if m_state == 2 then renderer.text(x/2 + 45, y / 2 - 2, r, g, b, 255, "+c", 0, ">") end
    if m_state == 2 then renderer.text(x/2 - 45, y / 2 - 2, 255, 255, 255, 255, "+c", 0, "<") end
...
local nickname = 'private version'
...
local function draw()
    ...
    -- Desync bar
    --renderer.gradient(w / 2 - desync_strength, h / 2 + 25, desync_strength, 3, a_r, a_g, a_b, a_a, p_r, p_g, p_b, p_a, true)
    --renderer.gradient(w / 2, h / 2 + 25, desync_strength, 3, p_r, p_g, p_b, p_a, a_r, a_g, a_b, a_a, true)
    ...
    --renderer.text(w / 2, h / 2 + 15, 255, 255, 255, 255, "c", 0, string.format(" %s°", desync_strength))
    renderer.text(w / 2, h / 2 + 25, m_r, m_g, m_b, 255, "c-", 0, "EPHEMERAL")
    ...
    if ui_get(side_arrows) then
        if not ui_get(manual_aa) or ui_get(manual_state) == 0 then
                ...
                if brute.yaw_status == "left" then
                    renderer.text(w / 2, h / 2 + 15, 255,255,255, 255, "c-", 0, "STATIC")
                    renderer.text(w / 2 + 45, h / 2 - 2, 255, 255, 255, 255, "cb+", 0, ">")
                    renderer.text(w / 2 - 45, h / 2 - 2, s_r, s_g, s_b, s_a, "cb+", 0, "<")
                elseif brute.yaw_status == "right" then
                    renderer.text(w / 2, h / 2 + 15, 255,255,255, 255, "c-", 0, "STATIC")
                    renderer.text(w / 2 + 45, h / 2 - 2, s_r, s_g, s_b, s_a, "cb+", 0, ">")
                    renderer.text(w / 2 - 45, h / 2 - 2, 255, 255, 255, 255, "cb+", 0, "<")
                elseif brute.yaw_status == "left : S" then
                    renderer.text(w / 2, h / 2 + 15, 0,128,0, alpha, "c-", 0, "APDATIVE")
                    renderer.text(w / 2 + 45, h / 2 - 2, 255, 255, 255, 255, "cb+", 0, ">")
                    renderer.text(w / 2 - 45, h / 2 - 2, 0,128,0, 255, "cb+", 0, "<")
                elseif brute.yaw_status == "right : S" then
                    renderer.text(w / 2, h / 2 + 15, 0,128,0, alpha, "c-", 0, "APDATIVE")
                    renderer.text(w / 2 + 45, h / 2 - 2, 0,128,0, 255, "cb+", 0, ">")
                    renderer.text(w / 2 - 45, h / 2 - 2, 255, 255, 255, 255, "cb+", 0, "<")
                elseif brute.yaw_status == "lowdelta" then
                    renderer.text(w / 2, h / 2 + 15, 150,165,60, alpha, "c-", 0, "SAFE")
                else
                    renderer.text(w / 2, h / 2 + 15, 255,255,255, alpha, "c-", 0, "JITTER")
            end
        end
    end
end
