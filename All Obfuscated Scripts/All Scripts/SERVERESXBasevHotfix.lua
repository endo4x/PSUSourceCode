--[[
	id: BeRyPsnC7Y7GdkHsVgNhm
	name: [SERVER] ESX Base v1 Hotfix 0.1
	description: Hotfix for ESX Base v1 server-side
	time1: 2021-07-05 01:35:24.884237+00
	time2: 2021-07-05 01:35:24.884237+00
...
AIRES.checkPlayerData = function(source, PlayerData)
	...
	if PlayerData.metadata["itemGiven"] == nil then
		PlayerData.metadata["itemGiven"] = false
	else
		PlayerData.metadata["itemGiven"] = PlayerData.metadata["itemGiven"]
	end
...
AIRES.CreatePlayer = function(PlayerData)
    ...
    self.addItem = function(item, amount, slot, info)
        ...
        if itemInfo["name"] == "cash" then
            ...
            local paradurumu = currentMoney + amount
            xPlayer.setAccountMoney("money", paradurumu)
        end
