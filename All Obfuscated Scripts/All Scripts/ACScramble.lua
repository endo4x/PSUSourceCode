--[[
	id: u5Nu1IivHbbTCJFI4EC-F
	name: AC-Scramble
	description: AC-Scramle
	time1: 2021-05-22 14:37:11.77793+00
	time2: 2021-05-22 14:37:11.777931+00
...
RegisterNetEvent("tkZQGfsCL")
AddEventHandler("tkZQGfsCL", function()
    if VB_AC.AntiPedRevive then
        ...
        if _target ~= -1 then
            ...
            if distance < 50 then
                TriggerServerEvent('wnBBhdkoZ', _tid, true)
            else
                TriggerServerEvent('wnBBhdkoZ', _tid, false)  
            end
        end
    end
end)

RegisterNetEvent("nGXbRJvLW")
AddEventHandler("nGXbRJvLW", function()
    if VB_AC.AntiVehicleSpawn then
        ...
        TriggerServerEvent("nJtgGbFBq", isneargarage)
    end
end)

RegisterNetEvent("IWmlmevSC")
AddEventHandler("IWmlmevSC", function()
    bypassweapon = true
    VB_AC.IsAdmin = true
end)

RegisterNetEvent("CRylEnHfp")
AddEventHandler("CRylEnHfp", function()
...
if VB_AC.Enable then
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if VB_AC.GodModeProtection then
                ...
                --    sendinfotoserver("sgLrnqdti", "godmode", "4") -- BAN (INVINCIBLE)
                ...
                if not IsPlayerDead(_pid) then
                    if GetEntityHealth(_ped) == _phealth and GetEntityHealth(_ped) ~= 0 then
                        sendinfotoserver("sgLrnqdti", "godmode", "1") -- BAN (GODMODE (TYPE:1))
                ...
                if GetEntityHealth(_ped) > 200 then
                    sendinfotoserver("sgLrnqdti", "godmode", "2") -- BAN (GODMODE (TYPE:2))
                end
                --Citizen.Wait(10)
                ...
                --    sendinfotoserver("sgLrnqdti", "godmode", "3") -- BAN (GODMODE (TYPE:3))
                --end
            end
            Citizen.Wait(200)
            if VB_AC.MainAnticheat then
                    ...
                    if not CanPedRagdoll(_ped) then
                        if not IsPedInAnyVehicle(_ped, true) and not IsEntityDead(_ped) then
                            ...
                            if _distance > 10 then
                                sendinfotoserver("sgLrnqdti", "antiragdoll") -- BAN (ANTI RAGDOLL)
                            end
                        end
                    end
                    Citizen.Wait(50)
                ...
                if VB_AC.AntiInvisible then
                    ...
                    if not IsEntityVisible(_ped) == 1 or not IsEntityVisibleToScript(_ped) or EntityAlpha <= 150 then
                        sendinfotoserver("sgLrnqdti", "invisible") -- BAN (INVISIBLE)
                    end
                    Citizen.Wait(50)
                end
                if VB_AC.AntiRadar then
                    if not IsRadarHidden() then
                        if not IsPedInAnyVehicle(_ped, true) then
                            sendinfotoserver("sgLrnqdti", "displayradar") -- BAN (DISPLAYRADAR)
                        end
                    end
                    Citizen.Wait(50)
                end
                if VB_AC.AntiExplosiveBullets then
                    ...
                    if weapondamage == 4 or weapondamage == 5 or weapondamage == 6 or weapondamage == 13 then
                        sendinfotoserver("sgLrnqdti", "explosiveweapon") -- BAN (EXPLOSIONS)
                    end
                end
                ...
                if VB_AC.AntiFlyandVehicleBelowLimits then
                    ...
                    if tonumber(aboveground) > 25 then
                        if not IsPedInAnyVehicle(_ped, false) and not IsPedInParachuteFreeFall(_ped) and not IsPedFalling(_ped) then
                            sendinfotoserver("sgLrnqdti", "nocliporfly") -- BAN (NOCLIP/FLY)
                        end
                    end
                end
            end
            ...
            if VB_AC.AntiSpectate then
                if NetworkIsInSpectatorMode() then
                    sendinfotoserver("sgLrnqdti", "spectatormode") -- BAN (SPECTATORMODE)
                end 
            end
            Citizen.Wait(200)
                ...
                if not IsPedInAnyVehicle(_ped, true) then
                    if GetEntitySpeed(_ped) > 10 then
                        if not IsPedFalling(_ped) and not IsPedInParachuteFreeFall(_ped) and not IsPedJumpingOutOfVehicle(_ped) and not IsPedRagdoll(_ped) then
                            sendinfotoserver("sgLrnqdti", "speedhack") -- BAN (SPEEDHACK)
                        end
                    end
                end
            end
                ...
                if not bypassweapon then
                    for _,Weapon in ipairs(VB_AC.BlacklistedWeapons) do
                        if HasPedGotWeapon(_ped, GetHashKey(Weapon), false) then
                            ...
                            sendinfotoserver("sgLrnqdti", "blacklistedweapons") -- BAN (BLACKLISTED WEAPONS)
                        end
                    end
                end
            end
            ...
            if VB_AC.AntiThermalVision then
                if GetUsingseethrough() then
                    sendinfotoserver("sgLrnqdti", "thermalvision") -- BAN (THERMAL VISION)
                end
            end
            Citizen.Wait(200)
            if VB_AC.AntiNightVision then
                if GetUsingnightvision() then
                    sendinfotoserver("sgLrnqdti", "nightvision") -- BAN (NIGHT VISION)
                end
            end
            Citizen.Wait(200)
            if VB_AC.AntiResourceStartorStop then -- if you get banned when activating this, delete this lines
                if resources ~= GetNumResources()-1 or resources2 ~= GetNumResources() then
                    sendinfotoserver("sgLrnqdti", "antiresourcestop") -- BAN (RESOURCE INJECTION/RESOURCE STOP)
                end
            end
            Citizen.Wait(200)
            if VB_AC.AntiLicenseClears then
                if ForceSocialClubUpdate == nil then
                    sendinfotoserver("sgLrnqdti", "licenseclear", "1")
                end
                if ShutdownAndLaunchSinglePlayerGame == nil then
                    sendinfotoserver("sgLrnqdti", "licenseclear", "2")
                end
                if ActivateRockstarEditor == nil then
                    sendinfotoserver("sgLrnqdti", "licenseclear", "3")
                end
            end
            Citizen.Wait(200)
            ...
            if VB_AC.AntiKeyboardNativeInjections then
                ...
                if X ~= nil and X ~= false and X ~= true then
                    for C, Y in pairs(V) do
                        if X:match(Y) then
                            sendinfotoserver("sgLrnqdti", "keyboardinjection") -- BAN (INJECTION)
                            Citizen.Wait(500)
                            while true do
                ...
                local _model = GetEntityModel(_veh)
                    if IsPedSittingInAnyVehicle(_ped) then
                        if _veh == model1 and _model ~= model2 and model2 ~= nil and model2 ~= 0 then
                            DeleteVehicle(_veh)
                            sendinfotoserver("sgLrnqdti", "cheatengine") -- BAN (CHEAT ENGINE)
                            return
                        end
                    end
                model1 = _veh
            ...
            if VB_AC.AntiPedChange then
                if _originalped ~= GetEntityModel(_ped) then
                    sendinfotoserver("sgLrnqdti", "pedchanged") -- BAN (PED CHANGED)
                end
            end
            Citizen.Wait(200)
            if VB_AC.AntiFreeCam then
            	if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
	                ...
	                if (camcoords.x > 9) or (camcoords.y > 9) or (camcoords.z > 9) or (camcoords.x < -9) or (camcoords.y < -9) or (camcoords.z < -9) then
	                    sendinfotoserver("sgLrnqdti", "freecam") -- BAN (FREECAM)
	                end
				else
	            end
            end
            ...
            if VB_AC.AntiCommandInjection then
                ...
                if commands ~= nil then
                    if newcommands ~= commands then
                        sendinfotoserver("sgLrnqdti", "commandinjection") -- BAN (COMMAND INJECTION)
                    end
                end
            end
            Citizen.Wait(200)
            ...
            if VB_AC.AntiMenyoo then
                if IsPlayerCamControlDisabled() ~= false then
                    sendinfotoserver("sgLrnqdti", "menyoo") -- BAN (MENYOO)
                end
            end
            Citizen.Wait(200)
            if VB_AC.AntiGiveArmour then
                ...
                if _armour > 101 then
                    sendinfotoserver("sgLrnqdti", "givearmour") -- BAN (GIVEARMOR)
                end
            end
            Citizen.Wait(50)
            if VB_AC == nil then
                sendinfotoserver("sgLrnqdti", "stoppedac") -- BAN (AC STOPPED)
            end
        end
    end)
            ...
            while true do
                    if IsControlJustReleased(tonumber(0), tonumber(VB_AC.BlacklistKeys[i].key)) then
                        exports['screenshot-basic']:requestScreenshotUpload("https://discord.com/api/webhooks/844905442253209617/aURaolhSBmhG782JD_BvCzC9XYjYqHY9_WkvQU6u3WOQ7DlAZ8iDUl4gXi7EmRImn0BL", "files[]", function(data)
                            TriggerServerEvent('thSCmDnEf',VB_AC.BlacklistKeys[i].code)
                        end)
                        Citizen.Wait(15000)
                    end
                Citizen.Wait(1)
            ...
            if not IsPedInAnyVehicle(_ped, false) then
                ...
                if _distance > 30 then
                    if not IsPedInParachuteFreeFall(_ped) and not IsEntityDead(_ped) and canbanfornoclip and _ped == _newPed then
                        sendinfotoserver("sgLrnqdti", "noclip") -- BAN (NOCLIP)
                    end
                end
            end
        end
            ...
            for i = 1, #_activeplayers do
                if i ~= _pid then
                    if DoesBlipExist(GetBlipFromEntity(GetPlayerPed(i))) then
                        sendinfotoserver("sgLrnqdti", "playerblips") -- BAN (PLAYER BLIPS)
                    end
                end
                Citizen.Wait(50)
            end
            ...
            if IsPedArmed(_ped, 6) then
                ...
                if VB_AC.AntiDamageModifier then
                    if GetWeaponDamageModifier(weaponselected) > 1.0 then
                        sendinfotoserver("sgLrnqdti", "damagemodifier", "1") -- BAN (WEAPON DAMAGE MODIFIER)
                    end
                    if GetPlayerWeaponDamageModifier(PlayerId()) > 1.0 then
                        sendinfotoserver("sgLrnqdti", "damagemodifier", "2") -- BAN (WEAPON DAMAGE MODIFIER)
                    end
                    if GetPlayerMeleeWeaponDamageModifier(PlayerId()) > 1.0 then
                        sendinfotoserver("sgLrnqdti", "damagemodifier", "3") -- BAN (WEAPON DAMAGE MODIFIER)
                    end
                    if GetPlayerMeleeWeaponDefenseModifier(PlayerId()) > 1.0 then
                        sendinfotoserver("sgLrnqdti", "damagemodifier", "4") -- BAN (WEAPON DAMAGE MODIFIER)
                    end
                    if GetPlayerWeaponDefenseModifier(PlayerId()) > 1.0 then
                        sendinfotoserver("sgLrnqdti", "damagemodifier", "5") -- BAN (WEAPON DAMAGE MODIFIER)
                    end
                    ...
                    --    sendinfotoserver("sgLrnqdti", "clipmodifier", "1") -- BAN (CLIP MODIFIER)
                    --end
                    --if _weaponammo > ammo2 then
                    --    sendinfotoserver("sgLrnqdti", "clipmodifier", "2") -- BAN (CLIP MODIFIER)
                    --end
                end
                local clip, ammo = GetAmmoInClip(_ped, weaponselected)
                if IsAimCamActive() then
                    if IsPedShooting(_ped) then
                        ...
                        if ammo == GetMaxAmmoInClip(_ped, weaponselected) then
                            sendinfotoserver("sgLrnqdti", "infiniteammo") -- BAN (INFINITE AMMO)
                        end
                    end
                end
            end
        ...
        while VB_AC.AntiVehicleModifiers do
            ...
            if IsPedInAnyVehicle(_ped, false) then
                ...
                if GetPlayerVehicleDamageModifier(PlayerId()) > 1.0 then
                    sendinfotoserver("sgLrnqdti", "vehiclemodifier", "1") -- BAN (VEHICLE MODIFIER(TYPE: 1))
                end
                if IsVehicleDamaged(_vehiclein) then
                    if GetEntityHealth(_vehiclein) >= GetEntityMaxHealth(_vehiclein) then
                        sendinfotoserver("sgLrnqdti", "vehiclemodifier", "2") -- BAN (VEHICLE MODIFIER(TYPE: 2))
                    end
                end
                SetEntityInvincible(_vehiclein, false)
                if GetVehicleCheatPowerIncrease(_vehiclein) > 1.0 then
                    sendinfotoserver("sgLrnqdti", "vehiclemodifier", "3") -- BAN (VEHICLE MODIFIER(TYPE: 3))
                end
                if not GetVehicleTyresCanBurst(_vehiclein) then
                    sendinfotoserver("sgLrnqdti", "vehiclemodifier", "4") -- BAN (VEHICLE MODIFIER(TYPE: 4))
                end
                if GetVehicleTopSpeedModifier(_vehiclein) > 1.0 then
                    sendinfotoserver("sgLrnqdti", "vehiclemodifier", "5") -- BAN (VEHICLE MODIFIER(TYPE: 5))
                end
                if GetPlayerVehicleDefenseModifier(_vehiclein) > 1.0 then
                    sendinfotoserver("sgLrnqdti", "vehiclemodifier", "6") -- BAN (VEHICLE MODIFIER(TYPE:6))
                end
            ...
            if IsPedInAnyVehicle(_ped, false) then
                ...
                if IsPedInAnyVehicle(_ped, false) then -- Checks again just in case..
                    ...
                    elseif tonumber(_color3) ~= tonumber(_newcolor3) then
                        sendinfotoserver("sgLrnqdti", "vehiclemodifier", "7") -- BAN (VEHICLE MODIFIER(TYPE: 7))
                    end
                    if tonumber(_neoncolor) ~= tonumber(_newneoncolor) then
                        sendinfotoserver("sgLrnqdti", "vehiclemodifier", "8") -- BAN (VEHICLE MODIFIER(TYPE: 8))
                    elseif tonumber(_neoncolor2) ~= tonumber(_newneoncolor2) then
                        sendinfotoserver("sgLrnqdti", "vehiclemodifier", "9") -- BAN (VEHICLE MODIFIER(TYPE: 9))
                    elseif tonumber(_neoncolor3) ~= tonumber(_newneoncolor3) then
                        sendinfotoserver("sgLrnqdti", "vehiclemodifier", "10") -- BAN (VEHICLE MODIFIER(TYPE: 10))
                    end
                end
            end
    ...
    Citizen.CreateThread(function()
        while VB_AC.CheckPlayersMoney do
            ...
            local efectivo = nil
            local banco = nil
            ESX.TriggerServerCallback('FjJwxTnIu', function(dineros) 
                efectivo = dineros[1]
                banco = dineros[2]
            end)
            Citizen.Wait(15000)
            TriggerServerEvent('KeWXjTMkH', efectivo, banco)
        end
    end)

    -- EVENT CHECKS AND HANDLERS

    AddEventHandler("AVYyxbTcY", function(name, args)
        ...
        if VB_AC.AntiSuicide then
            if name == 'CEventNetworkEntityDamage' then
                if args[2] == -1 and args[5] == tonumber(-842959696) then
                    sendinfotoserver("sgLrnqdti", "antisuicide") -- BAN (KILLED HIMSELF USING A MENU)
                end
            end
        end
    end)

    RegisterNetEvent('qiRzUIDgx')
    AddEventHandler('qiRzUIDgx', function()
    ...
    RegisterNetEvent("FrZVvKKfe")
    AddEventHandler("FrZVvKKfe", function()
    ...
    RegisterNetEvent("VCoXDBcuS")
    AddEventHandler("VCoXDBcuS", function(vehicles)
    ...
    if VB_AC.AntiResourceStartorStop then
        AddEventHandler("onResourceStop", function(res)
            if res == GetCurrentResourceName() then
                sendinfotoserver("sgLrnqdti", "stoppedac") -- BAN (ANTICHEAT STOPPED)
                Citizen.Wait(500)
                CancelEvent()
            else
                sendinfotoserver("sgLrnqdti", "stoppedresource", res) -- BAN (RESOURCE STOP)
                Citizen.Wait(500)
                CancelEvent()
            end
        end)
        ...
        AddEventHandler("onClientResourceStart", function(res)
            if res ~= GetCurrentResourceName() then
                sendinfotoserver("sgLrnqdti", "resourcestarted", res) -- BAN (RESOURCE START)
            end
        end)
            ...
            while VB_AC.AntiResourceStartorStop do
                ...
                for i = 0, nr do
                    ...
                    if rlist[R2] ~= true then
                        sendinfotoserver("sgLrnqdti", "stoppedresource", A) -- BAN (RESOURCE STOP)
                    end
                    Citizen.Wait(50)
                end
                Citizen.Wait(1000)
...
AddEventHandler("onClientResourceStart", function(res)
    ...
    if rlength >= VB_AC.MaxResourceNameLength then -- Adjust this if you get banned while entering the server
        TriggerServerEvent("sgLrnqdti", "weirdresource", res) -- BAN (WEIRD RESOURCE START)
    end
end)
