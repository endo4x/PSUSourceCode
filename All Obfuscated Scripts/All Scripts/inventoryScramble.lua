--[[
	id: aryT_4skjzw6MOzy-9XT2
	name: inventoryScramble
	description: inventoryScramble
	time1: 2021-05-22 14:31:56.980665+00
	time2: 2021-05-22 14:31:56.980665+00
...
RegisterNetEvent('sbcECgwQO')
AddEventHandler('sbcECgwQO', function(job)
...
AddEventHandler('njBggotmi', function()
	--Citizen.CreateThread(function()
...
RegisterNetEvent('zjPMtAuKi')
AddEventHandler('zjPMtAuKi', function(stat)
    if stat == true then
	    CanOpen2 = false
    else
        CanOpen2 = true
    ...
    while true do
        ...
        if IsControlJustReleased(0, Config.OpenControl) and IsInputDisabled(0) and not SlotDelay --[[and CanOpen]] then
            if not IsEntityInWater(GetPlayerPed(-1)) and not IsEntityDead(GetPlayerPed(-1)) then
                if CanOpen2 then
                    TriggerEvent('bDPEecUgy', 'GÃ¼venli Envanter', 'all')
                    DisplayRadar(false)
                    SlotDelay = true
                else
                    ESX.ShowNotification("AracÄ±n ~r~oluÅŸmadan ~w~ envanterini aÃ§amazsÄ±n.")
                end
            else
                ESX.ShowNotification("~b~Sudayken ~w~Envanter aÃ§amazsÄ±n.")
            end
        end
    end
end)

RegisterNetEvent('HKMytEPSk')
AddEventHandler('HKMytEPSk', function()
    closeInventory()
end)
...
function closeInventory()
    if secondary ~= nil then
        TriggerServerEvent('TithMHZys', secondary)
        secondary = nil
        DupeDelay = false
    end
...
RegisterNUICallback("UseItem", function(data, cb)
    if data.item.name ~= nil then
        TriggerServerEvent("sYocAWiGc", data.item.name)
    end
    ...
    function(data, cb)
       if IsPedSittingInAnyVehicle(playerPed) then
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("vZRtzRWHj", data.item.type, data.item.name, data.number)
...
RegisterNUICallback("GiveItem", function(data, cb)
    ...
    if foundPlayer then
        ...
        if data.item.type == "item_standard" then
            TriggerServerEvent("ruzznGfmc", data.player, "item_standard", data.item.name, count)
        end
        Wait(250)
        loadPlayerInventory()
        SetCurrentPedWeapon(PlayerPedId(), "WEAPON_UNARMED", true)
    else
        ESX.ShowNotification("YakÄ±nÄ±nda kimse yok!")
    end
    cb("ok")
end)
...
function loadPlayerInventory()
    ESX.TriggerServerCallback("fBNxQfVsx", function(data)
        items = {}
                                ...
                                if distance < 1.0 then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        ...
                                        if level >= v.PlayerLevel then
                                            ...
                                            ESX.ShowNotification("Bu market iÃ§in~y~ " ..v.PlayerLevel.." ~w~seviye olman lazÄ±m...")
                                        end
                                    end
                                end
                            end
...
OpenShopInventory = function(items, label)
    if openableself then
        ...
        ESX.TriggerServerCallback('atNkRZEtQ', function(table)
            SendNUIMessage({
        ...
        ESX.ShowNotification("Å?uan aÃ§amazsÄ±n!")
    end
end
...
RegisterNUICallback("TakeFromShop", function(data, cb)
    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        ESX.TriggerServerCallback("VFMUyhPMX", function(result)
            if result then
                loadPlayerInventory()
                cb("ok")
            end
...
RegisterNetEvent('bDPEecUgy')
AddEventHandler('bDPEecUgy', function(name, job)
    OpenStash(name, job)
end)

OpenStash = function(name, job)
    if openableself then
        ESX.TriggerServerCallback('QLYWAzBMd', function(table)
            SendNUIMessage({
                action = "setInfoText",
                text = name
            })
    ...
    --
        ...
        ESX.ShowNotification("EÅŸyalarÄ±na ÅŸuan bir baÅŸkasÄ± bakÄ±yor!")
    end
end
...
RegisterNUICallback("PutIntoStash",function(data, cb)
    ...
    if not CombatMode then
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            ESX.TriggerServerCallback("VROwZZCNR", function(result)
                if result ~= nil then
        ...
        ESX.ShowNotification("~r~Combat Mode~w~'dayken ~y~GÃ¼venli Envanteri ~w~kullanamazsÄ±n!")
    end
end)
    ...
    if not CombatMode then
        if not DupeDelay then
            ...
            if type(data.number) == "number" and math.floor(data.number) == data.number then
                ...
                ESX.TriggerServerCallback("euhpATHJs", function(result)
                    if result ~= nil then
                        refreshStash(secondary)
                        Wait(150)
                        loadPlayerInventory()
            ...
            ESX.ShowNotification("Bu ÅŸekilde ~r~Spam ~w~YapamazsÄ±n!")
            closeInventory()
        end
    else
        ESX.ShowNotification("~r~Combat Mode~w~'dayken ~y~GÃ¼venli Envanteri ~w~kullanamazsÄ±n!")
...
RegisterNetEvent('ZCkMhumoA')
AddEventHandler('ZCkMhumoA', function(item, count)
    --print(item)
...
RegisterNUICallback("TakeFromFast", function(data, cb)
    ...
    if Config.ItemWeapons.Enabled then
        if string.match(data.item.name, 'WEAPON_') then
            TriggerEvent('EandwRYRi', data.item.name)
            currentWeapon = nil
        end
    else
...
RegisterNetEvent('WtDMpCwim')
AddEventHandler('WtDMpCwim', function()
    for i = 1, 5, 1 do
        fastItems[i] = nil
    end
...
RegisterNetEvent('WlPnGCWDr')
AddEventHandler('WlPnGCWDr', function(variable)
            ...
            if IsDisabledControlJustReleased(1, 157) and not SlotDelay and not isInInventory and not IsEntityDead(GetPlayerPed(-1)) then
                if fastItems[1] ~= nil then
                    ...
                    if fastItems[1].name ~= nil then
                        TriggerServerEvent("sYocAWiGc", fastItems[1].name)
                        SlotDelay = true
            ...
            elseif IsDisabledControlJustReleased(1, 158) and not SlotDelay and not isInInventory and not IsEntityDead(GetPlayerPed(-1)) then
                if fastItems[2] ~= nil then
                    ...
                    if fastItems[2].name ~= nil then
                        TriggerServerEvent("sYocAWiGc", fastItems[2].name)
                        SlotDelay = true
            ...
            elseif IsDisabledControlJustReleased(1, 160) and not SlotDelay and not isInInventory and not IsEntityDead(GetPlayerPed(-1)) then
                if fastItems[3] ~= nil then
                    ...
                    if fastItems[3].name ~= nil then
                        TriggerServerEvent("sYocAWiGc", fastItems[3].name)
                        SlotDelay = true
            ...
            elseif IsDisabledControlJustReleased(1, 164) and not SlotDelay and not isInInventory and not IsEntityDead(GetPlayerPed(-1)) then
                if fastItems[4] ~= nil then
                    ...
                    if fastItems[4].name ~= nil then
                        TriggerServerEvent("sYocAWiGc", fastItems[4].name)
                        SlotDelay = true
            ...
            elseif IsDisabledControlJustReleased(1, 165) and not SlotDelay and not isInInventory and not IsEntityDead(GetPlayerPed(-1)) then
                if fastItems[5] ~= nil then
                    ...
                    if fastItems[5].name ~= nil then
                        TriggerServerEvent("sYocAWiGc", fastItems[5].name)
                        SlotDelay = true
...
RegisterCommand('komut',function(source,args)
	TriggerServerEvent('QLYWAzBMd')
end)

--item weapons
local currentWeapon

RegisterNetEvent('jCldINAxJ')
AddEventHandler('jCldINAxJ', function(weapon)
...
RegisterNetEvent('EandwRYRi')
AddEventHandler('EandwRYRi', function()
    if currentWeapon ~= nil then
        RemoveWeapon(currentWeapon)
        currentWeapon = nil
        ClearPedTasks(PlayerPedId())
...
function RemoveWeapon(weapon)
   ...
   -- TriggerServerEvent('ezkSTOPgI', hash, ammoCount)
...
function GiveWeapon(weapon)
    ...
    --ESX.TriggerServerCallback('ibTWKnECR', function(ammoCount)
        GiveWeaponToPed(playerPed, hash, 0, false, true)
        TriggerServerEvent('vXFiAKFNL')
