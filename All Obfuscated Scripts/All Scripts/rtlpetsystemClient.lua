--[[
	id: mG3gOIUqmvjtQWn9Pb1Tn
	name: rtl-petsystem Client
	description: rtl-petsystem Client
	time1: 2021-07-20 07:55:53.26186+00
	time2: 2021-07-20 07:55:53.261861+00
	uploader: H9tSJBHy2ogPoNjIvOVNy-BAnMco7EpJ6CskZ705
	uploadersession: yZzin3sTUEXmOE0eV-AkxJ_7hdYdtK
	flag: f
--]]
...
Citizen.CreateThread(function()
	...
	DoRequestModel(1794449327)  -- rottweiler
	DoRequestModel(307287994)  -- rottweiler
...
function OpenPetMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pet_menu', {
		...
		if data.current.value == 'come_animal' and come == 0 then
			ESX.TriggerServerCallback('rtl-petsystem:getPet', function(pet)
				if pet == 'chien' then
					model = -1788665315
					come = 1
					openchien()
				...
				elseif pet == 'chop' then
					...
					come = 1
					openchien()
				elseif pet == 'parduc' then
						model = 307287994
						come = 1
						openchien()
				else
					print('rtl-petsystem: unknown pet ' .. pet)
				end
			end)
			menu.close()
			...
			if distance < 5 then
				if count >= 1 then
					if status < 100 then
						...
						TriggerServerEvent('rtl-petsystem:consumePetFood')
						if status > 100 then
							status = 100
						end
						menu.close()
        ...
        elseif data.current.value == 'attack' then
                    if DoesEntityExist(ped) then
                        if not IsPedDeadOrDying(ped) then
                            if GetDistanceBetweenCoords(GetEntityCoords(ped), GetEntityCoords(PlayerPedId()), true) <= 10.0 then
                            ...
                            ESX.ShowNotification('A háziállatod meghalt')
                        end
                    else
                        ESX.ShowNotification('Nincsen háziállatod')
                    end			
...
Citizen.CreateThread(function()
	while true do
		...
		if status == 0 then
			TriggerServerEvent('rtl-petsystem:petDied')
			DeleteEntity(ped)
...
--[[
RegisterNetEvent("rtl-petsystem:openPet")
AddEventHandler("rtl-petsystem:openPet", function()
		OpenPetMenu()
end)
]]
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsControlJustReleased(0, 56) then -- F6
			OpenPetMenu()
		end
	end
end)
...
RegisterNUICallback('animal_buypp', function(data)
	if tonumber(data.pricepp) > 0 then
		ESX.TriggerServerCallback('rtl-petsystem:buyPetpp', function(boughtPed)
			if boughtPed then

			end
		end, data.pet, data.label, tonumber(data.pricepp))
	end
end)

RegisterNUICallback('animal_buy', function(data)
	if tonumber(data.price) > 0 then
		ESX.TriggerServerCallback('rtl-petsystem:buyPet', function(boughtPed)
			if boughtPed then
...
function OpenPetShop()
	for i=1, #Config.PetShop, 1 do
		SendNUIMessage({
			...
			pricepp = Config.PetShop[i].pricepp,			
			display = true,
		})
